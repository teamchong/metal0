...
13. **Use @group syntax for platform tests** - Use `@core1`, `@core2`, `@core3`, `@linux`, `@macos`, `@windows` to run platform-specific test groups. See "Test Runner" section below.
...
./zig-out/bin/metal0 test tests/cpython @core1 bool    # Core group + specific test
```

**Group files:** `tests/groups/{core1,core2,core3,linux,macos,windows}.txt`
...
**389 total tests** - NOT all can run on every platform. Categorized by dependencies:
...
Group files stored in: `tests/groups/{core1,core2,core3,linux,macos,windows}.txt`
...
| 397 | ~2-3min | If all compile successfully |
...
```yaml
jobs:
  # Core tests split into 3 parallel jobs
  test-cpython-core1:
  ...
  test-cpython-macos:
    ...
    steps:
      - run: ./zig-out/bin/metal0 test tests/cpython @macos  # 25 macOS tests
...
| Iterator/callback generic | CAREFUL | Use `comptime T: type` with trait bounds |
...
**Problem**: Zig comptime is O(n^2) - inline blocks get monomorphized per call site. 100 slice operations = 100 copies of inline logic = compilation timeout.
...
**Example - BAD vs GOOD:**
```zig
// BAD: 137 lines of inline comptime per slice (O(n^2))
...
# SOURCE CODE
src/
  build_dirs.zig              # Path utilities, project root detection, global cache
  ...
  analysis/                   # Type inference, traits
