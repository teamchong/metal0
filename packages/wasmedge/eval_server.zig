...
pub const EvalServer = struct {
    ...
    pub fn run(self: *EvalServer) !void {
        while (self.running) {
            const client_fd = std.posix.accept(self.server_fd, null, null, 0) catch |err| {
    ...
    fn executeBytecode(self: *EvalServer, bytecode: []const u8) ![]u8 {
        ...
        // Load bytecode VM module from cached bytes
        if (self.vm_wasm) |wasm| {
            try vm.loadFromBuffer(wasm);
        } else {
            return error.VMModuleNotLoaded;
        }
        try vm.validate();
        try vm.instantiate();
        ...
        try vm.execute("alloc", &.{
            wasmedge.Value.fromI32(@intCast(bytecode.len)),
        }, &alloc_results);
        ...
        try vm.execute("execute_bytecode", &.{
            wasmedge.Value.fromI32(wasm_ptr),
            wasmedge.Value.fromI32(@intCast(bytecode.len)),
        }, &exec_results);
