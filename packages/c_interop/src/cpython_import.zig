...
export fn PyImport_ReloadModule(module: *cpython.PyObject) callconv(.c) ?*cpython.PyObject {
    ...
    if (mod_obj.md_dict) |dict| {
        ...
        const dict_obj: *@import("pyobject_dict.zig").PyDictObject = @ptrCast(@alignCast(dict));
            ...
            if (result != module) {
                ...
                if (new_mod.md_dict) |new_dict| {
                    ...
                    if (mod_obj.md_dict) |old_dict| {
                        _ = @import("pyobject_dict.zig").PyDict_Update(old_dict, new_dict);
                    }
                }
...
export fn PyImport_ExecCodeModuleWithPathnames(
    ...
    if (@import("pyobject_none.zig").Py_None) |none| {
    ...
    const eval = @import("cpython_eval.zig");
