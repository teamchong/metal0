...
pub export fn PyGC_Collect() callconv(.c) isize {
...
pub export fn PyGC_Enable() callconv(.c) c_int {
    gc_enabled = true;
    return 0;
}

/// Disable automatic garbage collection
pub export fn PyGC_Disable() callconv(.c) c_int {
    gc_enabled = false;
    return 0;
}

/// Check if GC is enabled
pub export fn PyGC_IsEnabled() callconv(.c) c_int {
    return if (gc_enabled) 1 else 0;
}

/// Get current GC counts for each generation
pub export fn PyGC_GetCount(gen0: *isize, gen1: *isize, gen2: *isize) callconv(.c) void {
...
/// Get GC thresholds
pub export fn PyGC_GetThreshold(threshold0: *c_int, threshold1: *c_int, threshold2: *c_int) callconv(.c) void {
...
/// Set GC thresholds
pub export fn PyGC_SetThreshold(threshold0: c_int, threshold1: c_int, threshold2: c_int) callconv(.c) void {
...
/// Track object for GC
pub export fn PyObject_GC_Track(obj: *cpython.PyObject) callconv(.c) void {
    const objects = getTrackedObjects();
    objects.put(obj, {}) catch return;
}

/// Untrack object from GC
pub export fn PyObject_GC_UnTrack(obj: *cpython.PyObject) callconv(.c) void {
    const objects = getTrackedObjects();
    _ = objects.remove(obj);
}

/// Check if object is tracked
pub export fn PyObject_GC_IsTracked(obj: *cpython.PyObject) callconv(.c) c_int {
    const objects = getTrackedObjects();
    return if (objects.contains(obj)) 1 else 0;
}

/// Allocate GC-tracked object
pub export fn _PyObject_GC_New(type_obj: *cpython.PyTypeObject) callconv(.c) ?*cpython.PyObject {
...
pub export fn PyObject_GC_Del(obj: *cpython.PyObject) callconv(.c) void {
