...
pub export fn PyThread_acquire_lock(lock: ?*anyopaque, waitflag: c_int) callconv(.c) c_int {
    if (lock) |ptr| {
...
pub export fn PyThread_release_lock(lock: ?*anyopaque) callconv(.c) void {
    if (lock) |ptr| {
        const pymutex: *PyMutex = @ptrCast(@alignCast(ptr));
...
pub export fn PyThread_allocate_lock() callconv(.c) ?*anyopaque {
    const mutex = std.heap.c_allocator.create(PyMutex) catch return null;
    mutex.* = .{};
    return @ptrCast(mutex);
}

pub export fn PyThread_free_lock(lock: ?*anyopaque) callconv(.c) void {
    if (lock) |ptr| {
        const pymutex: *PyMutex = @ptrCast(@alignCast(ptr));
...
pub export fn PyThread_acquire_lock_timed(lock: ?*anyopaque, microseconds: i64, intr_flag: c_int) callconv(.c) c_int {
    _ = intr_flag;
    if (lock) |ptr| {
        const pymutex: *PyMutex = @ptrCast(@alignCast(ptr));
        if (microseconds == 0) {
...
pub export fn PyThread_create_key() callconv(.c) c_int {
...
pub export fn PyThread_delete_key(key: c_int) callconv(.c) void {
...
pub export fn PyThread_delete_key_value(key: c_int) callconv(.c) void {
    if (key > 0 and key < TLSStorage.MAX_KEYS) {
        TLSStorage.mutex.lock();
        defer TLSStorage.mutex.unlock();
        TLSStorage.key_values[@intCast(key)] = null;
...
pub export fn PyThread_get_key_value(key: c_int) callconv(.c) ?*anyopaque {
    if (key > 0 and key < TLSStorage.MAX_KEYS) {
        TLSStorage.mutex.lock();
        defer TLSStorage.mutex.unlock();
...
pub export fn PyThread_set_key_value(key: c_int, value: ?*anyopaque) callconv(.c) c_int {
    if (key > 0 and key < TLSStorage.MAX_KEYS) {
        TLSStorage.mutex.lock();
        defer TLSStorage.mutex.unlock();
        if (TLSStorage.key_valid[@intCast(key)]) {
...
pub export fn PyThread_ReInitTLS() callconv(.c) void {
    TLSStorage.mutex.lock();
    defer TLSStorage.mutex.unlock();
...
pub export fn PyThread_exit_thread() callconv(.c) void {
    // Can't directly exit a thread from here
}

pub export fn PyThread_get_stacksize() callconv(.c) usize {
    return 1024 * 1024; // 1MB default
}

pub export fn PyThread_set_stacksize(size: usize) callconv(.c) c_int {
    _ = size;
    return 0;
}

pub export fn PyThread_get_thread_ident() callconv(.c) c_ulong {
    const current = std.Thread.getCurrentId();
    return @intCast(current);
}

pub export fn PyThread_get_thread_native_id() callconv(.c) c_ulong {
    return PyThread_get_thread_ident();
}

pub export fn PyThread_GetInfo() callconv(.c) ?*cpython.PyObject {
...
pub export fn PyThread_init_thread() callconv(.c) void {}

pub export fn PyThread_start_new_thread(func: ?*const fn (?*anyopaque) callconv(.c) void, arg: ?*anyopaque) callconv(.c) c_ulong {
    if (func) |f| {
...
pub export fn PyTraceMalloc_Track(domain: c_uint, ptr: usize, size: usize) callconv(.c) c_int {
    _ = domain;
    _ = ptr;
    _ = size;
    return 0;
...
pub export fn PyTraceMalloc_Untrack(domain: c_uint, ptr: usize) callconv(.c) c_int {
    _ = domain;
    _ = ptr;
    return 0;
}
...
pub export fn PyOS_strtol(str: [*:0]const u8, ptr: ?*[*:0]u8, base: c_int) callconv(.c) c_long {
    return std.c.strtol(str, @ptrCast(ptr), base);
}

pub export fn PyOS_strtoul(str: [*:0]const u8, ptr: ?*[*:0]u8, base: c_int) callconv(.c) c_ulong {
