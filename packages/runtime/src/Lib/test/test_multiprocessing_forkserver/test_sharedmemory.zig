//! test.test_multiprocessing_forkserver.test_sharedmemory - Multiprocessing sharedmemory tests (forkserver mode)
...
pub const ResourceTracker = struct {
    ...
    pub fn register(self: *ResourceTracker, name: []const u8, kind: ResourceKind) !void {
        if (self.resources.get(name)) |*info| {
            _ = info;
            // Increment ref count
...
test "shared memory create" {
    ...
    var shm = try SharedMemory.create(allocator, "test_shm", 1024);
    defer shm.unlink();

    try std.testing.expectEqual(@as(usize, 1024), shm.size);
    try std.testing.expectEqualStrings("test_shm", shm.name);
...
test "posix shared memory" {
    var shm = try PosixSharedMemory.open_new("/test_posix", 4096);
