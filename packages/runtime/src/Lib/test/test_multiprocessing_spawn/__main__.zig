//! test.test_multiprocessing_spawn - multiprocessing_spawn functionality tests
...
test "multiprocessing_spawn_basic" {
    var ctx = TestContext.init(std.testing.allocator, "test_multiprocessing_spawn");
    try std.testing.expect(ctx.run());
}

test "multiprocessing_spawn_result" {
