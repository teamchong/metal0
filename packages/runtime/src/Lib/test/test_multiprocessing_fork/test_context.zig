//! test.test_multiprocessing_fork.test_context - Multiprocessing context tests (fork start method)
...
pub const StartMethod = enum {
    ...
    pub fn default() StartMethod {
        return .fork; // Fork is the default for this module
    }
};

/// Context for multiprocessing - determines how processes are started (fork method)
...
/// Get default context (fork method)
pub fn get_context(method: ?StartMethod) Context {
    return Context.init(std.heap.page_allocator, method orelse .fork);
}
...
test "context creation defaults to fork" {
    const ctx = get_context(null);
    try std.testing.expectEqual(StartMethod.fork, ctx.method);
}

test "context with specific method" {
    const ctx = get_context(.spawn);
    try std.testing.expectEqual(StartMethod.spawn, ctx.method);
}
...
test "default start method is fork" {
    const method = get_start_method();
    try std.testing.expectEqual(StartMethod.fork, method);
}
...
test "barrier type" {
    var barrier = BarrierType{ .parties = 3 };
    try std.testing.expectEqual(@as(usize, 0), barrier.count);
    _ = barrier.wait();
    try std.testing.expectEqual(@as(usize, 1), barrier.count);
    barrier.reset();
    try std.testing.expectEqual(@as(usize, 0), barrier.count);
}
