//! test.test_multiprocessing_fork.test_synchronize - Multiprocessing synchronize tests (fork start method)
const std = @import("std");

/// Lock for mutual exclusion (fork method)
...
test "rlock reentrant" {
    ...
    try std.testing.expectEqual(@as(usize, 1), rlock.count);

    // Can acquire again (reentrant)
    try std.testing.expect(rlock.acquire(true, null));
    try std.testing.expectEqual(@as(usize, 2), rlock.count);

    rlock.release();
    try std.testing.expectEqual(@as(usize, 1), rlock.count);

    rlock.release();
    try std.testing.expectEqual(@as(usize, 0), rlock.count);
}
...
test "event wait returns current state" {
    var event = Event{};
    try std.testing.expect(!event.wait(null));
