//! test.test_multiprocessing_fork.test_queue - Multiprocessing queue tests (fork start method)
...
/// Thread-safe queue for multiprocessing (fork method)
...
test "simple queue alias" {
    const allocator = std.testing.allocator;
    var q = SimpleQueue(i32).init(allocator, null);
    defer q.deinit();

    try q.put(42, true, null);
    const v = try q.get_nowait();
...
test "queue empty error" {
    ...
    try std.testing.expectError(QueueError.Empty, q.get_nowait());
}
