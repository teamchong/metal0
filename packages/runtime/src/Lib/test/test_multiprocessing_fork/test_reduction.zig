//! test.test_multiprocessing_fork.test_reduction - Multiprocessing reduction tests (fork start method)
const std = @import("std");

/// Reduction registry for pickling objects (fork method - simpler as memory is shared)
...
test "dump and load integer" {
    const allocator = std.testing.allocator;

    const value: i64 = -123456789;
    const data = try dump(value, allocator);
    defer allocator.free(data);

    const restored = try load(i64, data);
    try std.testing.expectEqual(@as(i64, -123456789), restored);
}

test "abstract reducer with struct" {
    const allocator = std.testing.allocator;

    const Color = struct { r: u8, g: u8, b: u8, a: u8 };
    const Reducer = AbstractReducer(Color);

    const c = Color{ .r = 255, .g = 128, .b = 64, .a = 255 };
    const data = try Reducer.reduce(c, allocator);
    defer allocator.free(data);

    const restored = try Reducer.reconstruct(data);
    try std.testing.expectEqual(@as(u8, 255), restored.r);
    try std.testing.expectEqual(@as(u8, 128), restored.g);
    try std.testing.expectEqual(@as(u8, 64), restored.b);
}

test "reduce registry init deinit" {
    const allocator = std.testing.allocator;
    var registry = ReduceRegistry.init(allocator);
    defer registry.deinit();

    try std.testing.expectEqual(@as(usize, 0), registry.reducers.count());
}
