//! test.test_multiprocessing_fork.test_connection - Multiprocessing connection tests (fork start method)
...
pub const Family = enum {
    ...
    pub fn default() Family {
        return .AF_UNIX; // Unix sockets preferred for fork method
    }
};

/// Connection for inter-process communication (fork method)
pub const Connection = struct {
    readable: bool = true,
    writable: bool = true,
    closed: bool = false,
    handle: ?i32 = null,
    family: Family = .AF_UNIX,
...
/// Pipe - returns connected pair of connections (fork method uses OS pipes)
pub const Pipe = struct {
    pub fn create(allocator: std.mem.Allocator, duplex: bool) !struct { *Connection, *Connection } {
        ...
        // Set default family for fork method
        conn1.family = .AF_UNIX;
        conn2.family = .AF_UNIX;
...
pub fn Client(address: []const u8, family: ?Family, authkey: ?[]const u8) !Connection {
    ...
    conn.family = family orelse .AF_UNIX;
    _ = address;
    return conn;
}
...
test "pipe duplex" {
    ...
    try std.testing.expectEqual(Family.AF_UNIX, conn1.family);
}
...
test "listener accept" {
    const allocator = std.testing.allocator;
    var listener = Listener.init(allocator, "/tmp/mp-test", null);
    ...
    try std.testing.expectEqual(Family.AF_UNIX, conn.family);

    listener.close();
    try std.testing.expectError(error.ListenerClosed, listener.accept());
}

test "default family is unix" {
    try std.testing.expectEqual(Family.AF_UNIX, Family.default());
    try std.testing.expectEqual(Family.AF_UNIX, address.default_family());
}

test "connection fileno" {
    const allocator = std.testing.allocator;
    var conn = Connection.init(allocator, 42);
    defer conn.deinit();

    try std.testing.expectEqual(@as(?i32, 42), conn.fileno());

    conn.close();
    try std.testing.expect(conn.fileno() == null);
}

test "send bytes alias" {
    const allocator = std.testing.allocator;
    var conn = Connection.init(allocator, 1);
    defer conn.deinit();

    try conn.send_bytes("test data");
    const data = try conn.recv_bytes(null);
    defer allocator.free(data);
