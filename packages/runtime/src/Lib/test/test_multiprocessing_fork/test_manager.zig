//! test.test_multiprocessing_fork.test_manager - Multiprocessing manager tests (fork start method)
const std = @import("std");

/// Manager for sharing data between processes (fork method)
...
test "managed namespace" {
    const allocator = std.testing.allocator;
    var mgr = Manager.init(allocator);
    try mgr.start();
    defer mgr.shutdown();

    var ns = try mgr.Namespace();
    defer ns.deinit();

    try ns.setAttr("foo", "bar");
    try std.testing.expectEqualStrings("bar", ns.getAttr("foo").?);
}
...
test "managed queue" {
    ...
    try std.testing.expectEqualStrings("item1", item);
}

test "manager not started error" {
    const allocator = std.testing.allocator;
    var mgr = Manager.init(allocator);

    try std.testing.expectError(error.ManagerNotStarted, mgr.dict());
}

test "sync manager" {
    var sm = SyncManager.init("localhost:5000", null);
    try std.testing.expect(!sm.connected);
