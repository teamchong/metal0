//! test.test_multiprocessing_fork - multiprocessing_fork functionality tests
...
test "multiprocessing_fork_basic" {
    var ctx = TestContext.init(std.testing.allocator, "test_multiprocessing_fork");
    try std.testing.expect(ctx.run());
}

test "multiprocessing_fork_result" {
