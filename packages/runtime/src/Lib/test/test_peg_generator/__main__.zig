//! test.test_peg_generator - peg_generator functionality tests
...
test "peg_generator_basic" {
    var ctx = TestContext.init(std.testing.allocator, "test_peg_generator");
    try std.testing.expect(ctx.run());
}

test "peg_generator_result" {
