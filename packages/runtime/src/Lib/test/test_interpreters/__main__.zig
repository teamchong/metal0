//! test.test_interpreters - interpreters functionality tests
...
test "interpreters_basic" {
    var ctx = TestContext.init(std.testing.allocator, "test_interpreters");
    try std.testing.expect(ctx.run());
}

test "interpreters_result" {
