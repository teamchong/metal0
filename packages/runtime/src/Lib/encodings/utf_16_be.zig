...
//!
//! Mirrors: CPython Lib/encodings/utf_16_be.py

const std = @import("std");
const charmap = @import("charmap.zig");

pub const name = "utf-16-be";
pub const aliases = [_][]const u8{ "utf_16_be", "UTF-16BE" };
...
/// Decode UTF-16-BE bytes to UTF-8
pub fn decode(allocator: std.mem.Allocator, input: []const u8, errors: ErrorHandler) !DecodeResult {
    ...
    var i: usize = 0;
    while (i + 1 < input.len) {
        const code_unit = (@as(u16, input[i]) << 8) | @as(u16, input[i + 1]);
        i += 2;
        ...
        if (code_unit >= 0xD800 and code_unit <= 0xDBFF) {
            ...
            const low = (@as(u16, input[i]) << 8) | @as(u16, input[i + 1]);
            i += 2;
...
/// Encode UTF-8 string to UTF-16-BE bytes
pub fn encode(allocator: std.mem.Allocator, input: []const u8, errors: ErrorHandler) !EncodeResult {
    ...
    while (i < input.len) {
        ...
        if (codepoint < 0x10000) {
            ...
            try output.appendSlice(allocator, &[_]u8{ @truncate(cu >> 8), @truncate(cu) });
        } else {
            // Supplementary - surrogate pair
            const adjusted = codepoint - 0x10000;
            const high: u16 = @intCast((adjusted >> 10) + 0xD800);
            const low: u16 = @intCast((adjusted & 0x3FF) + 0xDC00);
            try output.appendSlice(allocator, &[_]u8{ @truncate(high >> 8), @truncate(high) });
            try output.appendSlice(allocator, &[_]u8{ @truncate(low >> 8), @truncate(low) });
        }
    }
...
test "utf16be decode ascii" {
    const result = try decode(std.testing.allocator, "\x00H\x00i", .strict);
    defer std.testing.allocator.free(result.output);
    try std.testing.expectEqualStrings("Hi", result.output);
}

test "utf16be encode ascii" {
    const result = try encode(std.testing.allocator, "Hi", .strict);
    defer std.testing.allocator.free(result.output);
    try std.testing.expectEqualStrings("\x00H\x00i", result.output);
}
