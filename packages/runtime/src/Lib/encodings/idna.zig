...
pub fn decode(allocator: std.mem.Allocator, input: []const u8, mode: ErrorMode) !DecodeResult {
    ...
    while (labels.next()) |label| {
        ...
        if (std.mem.startsWith(u8, label, ACE_PREFIX)) {
            ...
            try result.appendSlice(allocator, puny_result.output);
        } else {
            // Already ASCII
...
pub fn encode(allocator: std.mem.Allocator, input: []const u8, mode: ErrorMode) !EncodeResult {
    ...
    while (i < input.len) {
        ...
        // Check for ASCII dot
        if (byte == '.') {
            try labels.append(allocator, input[start..i]);
            start = i + 1;
            i += 1;
            continue;
        }
    ...
    if (start <= input.len) {
        try labels.append(allocator, input[start..]);
    }
    ...
    for (labels.items) |label| {
        ...
        if (all_ascii) {
            ...
            try result.appendSlice(allocator, ACE_PREFIX);
