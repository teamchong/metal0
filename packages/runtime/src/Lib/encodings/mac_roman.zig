...
//!
//! Mirrors: CPython Lib/encodings/mac_roman.py

const std = @import("std");
const charmap = @import("charmap.zig");

pub const name = "mac-roman";
pub const aliases = [_][]const u8{ "mac_roman", "macroman", "macintosh" };

const UNDEF = charmap.UNDEFINED;

/// Mac Roman decode table
const decode_table: [256]u21 = blk: {
    ...
    table[0xDE] = 0xFB01; // LATIN SMALL LIGATURE FI
    table[0xDF] = 0xFB02; // LATIN SMALL LIGATURE FL
    // 0xE0-0xFF
    ...
    table[0xF0] = 0xF8FF; // Apple logo (Private Use Area)
