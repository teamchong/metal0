...
pub fn get_importer(path_item: []const u8) ?*anyopaque {
    // Would check sys.path_hooks
    _ = path_item;
    return null;
}
...
/// Find loader for a module
pub fn find_loader(fullname: []const u8, path: ?[]const []const u8) !?struct { loader: ?*anyopaque, portions: []const []const u8 } {
    _ = fullname;
    _ = path;
    // Would use importlib machinery
    return null;
}
...
/// Get loader for a module
pub fn get_loader(module_or_name: []const u8) ?*anyopaque {
    _ = module_or_name;
    // Would use importlib
    return null;
}
