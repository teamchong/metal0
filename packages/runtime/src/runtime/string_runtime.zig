...
pub fn strRepeat(allocator: std.mem.Allocator, s: anytype, n: usize) []const u8 {
    // Extract the actual slice from either []const u8, []u8, PyBytes, or string literal pointer at comptime type check
    const T = @TypeOf(s);
    const actual_slice: []const u8 = if (T == []const u8)
        s
    else if (T == []u8)
        // Mutable slices are also valid - just use them as const
        s
