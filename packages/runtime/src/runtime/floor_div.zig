...
/// Returns i64 for integer types - callers handle float conversion if needed
...
/// Internal dispatch for floor division - returns i64
fn dispatchFloorDiv(a: anytype, b: anytype) i64 {
    ...
    // Convert to f64 for mixed or float types, then floor and return as i64
    const af: f64 = if (type_predicates.isIntInfo(info_a)) @floatFromInt(a) else @floatCast(a);
    const bf: f64 = if (type_predicates.isIntInfo(info_b)) @floatFromInt(b) else @floatCast(b);
    return @intFromFloat(@floor(af / bf));
}
