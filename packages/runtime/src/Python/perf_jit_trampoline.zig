...
pub const PerfJITManager = struct {
    ...
    /// For perf profiling, we store the function name as a simple string.
    /// Full DWARF debug info is generated by the Zig compiler at build time
    /// when using -fDebug flag, so we don't duplicate that effort here.
    fn generateDebugInfo(
        ...
        // Store function name for perf symbol resolution
        // Full DWARF is generated at build time by Zig compiler
