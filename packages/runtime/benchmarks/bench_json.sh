...
# Build metal0 parse benchmark (use pre-built binary from build system)
echo "🔨 Building metal0 parse benchmark..."
metal0_ROOT="$(cd ../../.. && pwd)"
cd "$metal0_ROOT" && zig build -Doptimize=ReleaseFast && cd - >/dev/null
if [ -f "$metal0_ROOT/zig-out/bin/bench_metal0_json_parse" ]; then
    ln -sf "$metal0_ROOT/zig-out/bin/bench_metal0_json_parse" /tmp/bench_metal0_json_parse
    echo "✅ metal0 parse benchmark built (C allocator)"
    metal0_AVAILABLE=true
else
    echo "⚠️  metal0 build failed"
...
if [ "$metal0_AVAILABLE" = true ]; then
    PARSE_CMD+=(--command-name "metal0" "/tmp/bench_metal0_json_parse")
fi
...
# Use pre-built metal0 stringify benchmark
echo "🔨 Using metal0 stringify benchmark..."
if [ -f "$metal0_ROOT/zig-out/bin/bench_metal0_json_stringify" ]; then
    ln -sf "$metal0_ROOT/zig-out/bin/bench_metal0_json_stringify" /tmp/bench_metal0_json_stringify
    echo "✅ metal0 stringify benchmark ready (C allocator)"
    metal0_STRINGIFY_AVAILABLE=true
else
    echo "⚠️  metal0 stringify benchmark not found"
    metal0_STRINGIFY_AVAILABLE=false
fi
...
if [ "$metal0_STRINGIFY_AVAILABLE" = true ]; then
    STRINGIFY_CMD+=(--command-name "metal0" "/tmp/bench_metal0_json_stringify")
fi
