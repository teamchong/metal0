...
pub fn genEpoll(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn fromfd(self: *@This(), fd: i32) void { __self._epfd = fd; }\n");
    try self.emitIndent();
    try self.emit("pub fn register(self: *@This(), fd: i64, eventmask: ?u32) void { _ = fd; _ = eventmask; }\n");
    try self.emitIndent();
    try self.emit("pub fn modify(self: *@This(), fd: i64, eventmask: u32) void { _ = fd; _ = eventmask; }\n");
    try self.emitIndent();
    try self.emit("pub fn unregister(self: *@This(), fd: i64) void { _ = fd; }\n");
    try self.emitIndent();
...
pub fn genDevpoll(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn register(self: *@This(), fd: i64, eventmask: ?i16) void { _ = fd; _ = eventmask; }\n");
    try self.emitIndent();
    try self.emit("pub fn modify(self: *@This(), fd: i64, eventmask: i16) void { _ = fd; _ = eventmask; }\n");
    try self.emitIndent();
    try self.emit("pub fn unregister(self: *@This(), fd: i64) void { _ = fd; }\n");
    try self.emitIndent();
    try self.emit("pub fn poll(self: *@This(), timeout: ?f64) []struct { i64, i16 } { _ = timeout; return &.{}; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
...
pub fn genKqueue(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn fromfd(self: *@This(), fd: i32) void { __self._kq = fd; }\n");
    try self.emitIndent();
