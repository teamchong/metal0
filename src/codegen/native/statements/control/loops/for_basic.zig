...
fn genTupleUnpackLoop(self: *NativeCodegen, target: ast.Node, iter: ast.Node, body: []ast.Node, orelse_body: ?[]ast.Node) CodegenError!void {
    ...
    if (needs_lazy_wrapper) {
        ...
        if (is_slice) {
            ...
            try self.withInlineBlock("iter", iter, struct {
                fn emit(s: *NativeCodegen, label: []const u8, i: ast.Node) CodegenError!void {
                    try s.emit(" const __iter = ");
                    try s.genExpr(i);
                    try s.emitFmt("; break :{s} __iter.items", .{label});
                }
            }.emit);
        } else {
    ...
    if (container_traits.isTuple(iter_type)) {
        ...
        if (should_unroll) {
            ...
            for (tuple_elts) |tuple_elt| {
                ...
                // Emit discard to prevent "unused variable" errors when body uses VM fallback
    ...
    if (is_constant_array or is_array_var) {
        ...
        if (container_traits.isList(iter_type) or iter_type == .deque) {
            ...
            if (is_slice) {
                ...
                try self.withInlineBlock("iter", for_stmt.iter.*, struct {
                    fn emit(s: *NativeCodegen, label: []const u8, i: ast.Node) CodegenError!void {
                        try s.emit(" const __iter = ");
                        try s.genExpr(i);
                        try s.emitFmt("; break :{s} __iter.items", .{label});
