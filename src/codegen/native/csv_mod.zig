...
fn genReader(self: *h.NativeCodegen, args: []ast.Node) h.CodegenError!void {
    ...
    try self.emitFmt("(__m{d}_rd: {{ const _f = ", .{id});
    try self.genExpr(args[0]);
    try self.emit("; const _d: u8 = ','; const _str = if (@TypeOf(_f) == *runtime.io.StringIO) _f.getvalue() else _f; break :__m");
    try self.emitFmt("{d}_rd struct {{ data: []const u8, pos: usize = 0, delim: u8, pub fn next(s: *@This()) ?[][]const u8 {{ if (s.pos >= s.data.len) return null; const le = std.mem.indexOfScalarPos(u8, s.data, s.pos, '\\n') orelse s.data.len; const ln = s.data[s.pos..le]; s.pos = le + 1; var fs: std.ArrayList([]const u8) = .{{}}; var it = std.mem.splitScalar(u8, ln, s.delim); while (it.next()) |f| fs.append(__global_allocator, f) catch continue; return fs.items; }} }}{{ .data = _str, .delim = _d }}; }})", .{id});
}
