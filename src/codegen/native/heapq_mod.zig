...
fn genHeappush(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("hpush", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try b.write("var _heap = ");
            const output1 = b.getBodyAndClear();
            try c.output.appendSlice(c.allocator, output1);
            try c.genExpr(a[0]);
            {
                const b2 = try c.getBuilder();
                try b2.write("; const _item = ");
            ...
            try c.genExpr(a[1]);
            {
                ...
                try b3.writeFmt("; _heap.append(__global_allocator, _item) catch unreachable; var _i = _heap.items.len - 1; while (_i > 0) {{ const _parent = (_i - 1) / 2; if (_heap.items[_i] >= _heap.items[_parent]) break; const tmp = _heap.items[_i]; _heap.items[_i] = _heap.items[_parent]; _heap.items[_parent] = tmp; _i = _parent; }} break :{s}; ", .{label});
...
fn genHeappop(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("hpop", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; const _h = if (@typeInfo(@TypeOf(_heap)) == .@\"struct\" and @hasField(@TypeOf(_heap), \"items\")) _heap.items else &_heap; if (_h.len == 0) break :{s} @as(@TypeOf(_h[0]), undefined); const _result = _h[0]; break :{s} _result; ", .{ label, label });
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
        }
...
fn genHeapreplace(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("hrep", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b3 = try c.getBuilder();
                try b3.writeFmt("; if (_heap.items.len == 0) break :{s} _item; const _result = _heap.items[0]; _heap.items[0] = _item; break :{s} _result; ", .{ label, label });
                const output3 = b3.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output3);
            }
        }
...
fn genHeappushpop(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("hpp", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b3 = try c.getBuilder();
                try b3.writeFmt("; if (_heap.items.len == 0 or _item <= _heap.items[0]) break :{s} _item; const _result = _heap.items[0]; _heap.items[0] = _item; break :{s} _result; ", .{ label, label });
                const output3 = b3.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output3);
            }
        }
...
fn genNlargest(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("nlrg", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            const b = try c.getBuilder();
            try b.write("const _n: usize = @intCast(");
            const output1 = b.getBodyAndClear();
            try c.output.appendSlice(c.allocator, output1);
            try c.genExpr(a[0]);
            {
                const b2 = try c.getBuilder();
                try b2.write("); const _items = ");
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
            try c.genExpr(a[1]);
            {
                const b3 = try c.getBuilder();
                try b3.writeFmt(".items; var _sorted = __global_allocator.alloc(@TypeOf(_items[0]), _items.len) catch break :{s} &[_]@TypeOf(_items[0]){{}}; @memcpy(_sorted, _items); std.mem.sort(@TypeOf(_items[0]), _sorted, {{}}, struct {{ fn cmp(_: void, a1: anytype, b1: anytype) bool {{ return a1 > b1; }} }}.cmp); break :{s} _sorted[0..@min(_n, _sorted.len)]; ", .{ label, label });
                const output3 = b3.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output3);
            }
        }
