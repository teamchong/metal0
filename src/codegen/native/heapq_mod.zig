...
const heappushBody = "; _heap.append(__global_allocator, _item) catch unreachable; var _i = _heap.items.len - 1; while (_i > 0) { const _parent = (_i - 1) / 2; if (_heap.items[_i] >= _heap.items[_parent]) break; const tmp = _heap.items[_i]; _heap.items[_i] = _heap.items[_parent]; _heap.items[_parent] = tmp; _i = _parent; } break :blk; }";
