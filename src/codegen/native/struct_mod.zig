...
pub fn genCalcsize(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("struct_calcsize_blk: { const _fmt = "); try self.genExpr(args[0]);
    try self.emit("; var _size: usize = 0; for (_fmt) |c| { _size += switch (c) { 'b', 'B', 'c', '?', 'x' => 1, 'h', 'H' => 2, 'i', 'I', 'l', 'L', 'f' => 4, 'q', 'Q', 'd' => 8, else => 0 }; } break :struct_calcsize_blk @as(i64, @intCast(_size)); }");
}
...
pub fn genIterUnpack(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len < 2) return;
    try self.emit("struct_iter_unpack_blk: { const _fmt = "); try self.genExpr(args[0]);
    try self.emit("; const _data = "); try self.genExpr(args[1]);
    try self.emit("; _ = _fmt; _ = _data; break :struct_iter_unpack_blk struct { items: []const u8, pos: usize = 0, pub fn next(__self: *@This()) ?i32 { if (__self.pos + 4 <= __self.items.len) { const val = std.mem.bytesToValue(i32, __self.items[__self.pos..][0..4]); __self.pos += 4; return val; } return null; } }{ .items = _data }; }");
}
