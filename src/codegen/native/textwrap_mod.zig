...
// Note: These are static string constants used in h.wrap() calls below
// They use descriptive static labels since they can't use runtime nextNameId()
const dedentBody = "; var _min_indent: usize = std.math.maxInt(usize); var _lines_iter = std.mem.splitSequence(u8, _text, \"\\n\"); while (_lines_iter.next()) |line| { if (line.len == 0) continue; var _spaces: usize = 0; for (line) |c| { if (c == ' ') _spaces += 1 else break; } if (_spaces < _min_indent) _min_indent = _spaces; } if (_min_indent == std.math.maxInt(usize)) _min_indent = 0; var _result = std.ArrayListUnmanaged(u8){}; var _lines_iter2 = std.mem.splitSequence(u8, _text, \"\\n\"); var _first = true; while (_lines_iter2.next()) |line| { if (!_first) _result.append(__global_allocator, '\\n') catch continue; _first = false; if (line.len > _min_indent) _result.appendSlice(__global_allocator, line[_min_indent..]) catch continue; } break :__m_dedent _result.items; }";
...
pub const Funcs = std.StaticStringMap(h.H).initComptime(.{
    .{ "wrap", genWrap }, .{ "fill", genFill },
    .{ "dedent", h.wrap("__m_dedent: { const _text = ", dedentBody, "\"\"") },
    .{ "indent", h.wrap2("__m_indent: { const _text = ", "; const _prefix = ", indentBody, "\"\"") },
    .{ "shorten", h.wrap2("__m_shorten: { const _text = ", "; const _width: usize = @intCast(", shortenBody, "\"\"") },
...
fn genWidth(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    if (args.len > 1) { try self.emit("@intCast("); try self.genExpr(args[1]); try self.emit(")"); } else try self.emit("70");
}
...
fn genWrap(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("wrap", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try c.emit("const _text = ");
            try c.genExpr(a[0]);
            try genWidth(c, a);
            try c.emitFmt("; var _lines = std.ArrayListUnmanaged([]const u8){{}}; var _start: usize = 0; while (_start < _text.len) {{ const _end = @min(_start + _width, _text.len); _lines.append(__global_allocator, _text[_start.._end]) catch continue; _start = _end; }} break :{s} _lines.items", .{label});
        }
    }.emit);
}
...
fn genFill(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("fill", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[0]);
            try genWidth(c, a);
            try c.emitFmt("; var _result = std.ArrayListUnmanaged(u8){{}}; var _start: usize = 0; while (_start < _text.len) {{ const _end = @min(_start + _width, _text.len); if (_start > 0) _result.append(__global_allocator, '\\n') catch continue; _result.appendSlice(__global_allocator, _text[_start.._end]) catch continue; _start = _end; }} break :{s} _result.items", .{label});
        }
    }.emit);
}
