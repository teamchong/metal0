...
pub fn genTextWrapper(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn wrap(self: *@This(), text: []const u8) [][]const u8 { _ = self; _ = text; return &.{}; }\n");
    try self.emitIndent();
    try self.emit("pub fn fill(self: *@This(), text: []const u8) []const u8 { _ = self; return text; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
