...
fn genGlob(self: *h.NativeCodegen, args: []ast.Node) h.CodegenError!void {
    ...
    try self.emitFmt("(__m{d}_glob: {{ const _pattern = ", .{id});
    try self.genExpr(args[0]);
    try self.emit("; var _results: std.ArrayList([]const u8) = .{}; const _dir_path = std.fs.path.dirname(_pattern) orelse \".\"; const _file_pattern = std.fs.path.basename(_pattern); var _dir = std.fs.cwd().openDir(_dir_path, .{ .iterate = true }) catch break :__m");
    try self.emitFmt("{d}_glob _results.items; defer _dir.close(); var _iter = _dir.iterate(); while (_iter.next() catch null) |entry| {{ var _gmatch = true; var _gpi: usize = 0; var _gni: usize = 0; var _gstar_pi: ?usize = null; var _gstar_ni: usize = 0; __m{d}_gml: while (_gni < entry.name.len or _gpi < _file_pattern.len) {{ if (_gpi < _file_pattern.len) {{ const _gpc = _file_pattern[_gpi]; if (_gpc == '*') {{ _gstar_pi = _gpi; _gstar_ni = _gni; _gpi += 1; continue; }} if (_gni < entry.name.len and (_gpc == '?' or _gpc == entry.name[_gni])) {{ _gpi += 1; _gni += 1; continue; }} }} if (_gstar_pi) |_gsp| {{ _gpi = _gsp + 1; _gstar_ni += 1; _gni = _gstar_ni; if (_gni <= entry.name.len) continue; }} _gmatch = false; break :__m{d}_gml; }} if (_gmatch) {{ const _full = std.fmt.allocPrint(__global_allocator, \"", .{ id, id, id });
