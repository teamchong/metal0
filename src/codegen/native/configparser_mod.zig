...
pub fn genConfigParser(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn read(__self: *@This(), filename: []const u8) !void {\n");
    self.indent();
    try self.emitIndent();
    ...
    try self.emitIndent();
    try self.emit("if (__self.sections_map.get(current_section.?) == null) {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("__self.sections_map.put(current_section.?, hashmap_helper.StringHashMap([]const u8).init(__global_allocator)) catch continue;\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    self.dedent();
    ...
    try self.emitIndent();
    try self.emit("if (__self.sections_map.getPtr(current_section.?)) |sec| sec.put(key, value) catch {};\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    self.dedent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn read_string(__self: *@This(), content: []const u8) void {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("var current_section: ?[]const u8 = null;\n");
    try self.emitIndent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn sections(__self: *@This()) [][]const u8 {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("var result: std.ArrayList([]const u8) = .{};\n");
    try self.emitIndent();
    try self.emit("var iter = __self.sections_map.keyIterator();\n");
    try self.emitIndent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn has_section(__self: *@This(), section: []const u8) bool {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("return __self.sections_map.get(section) != null;\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn get(__self: *@This(), section: []const u8, option: []const u8) ?[]const u8 {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("if (__self.sections_map.get(section)) |sec| return sec.get(option);\n");
    try self.emitIndent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn getint(__self: *@This(), section: []const u8, option: []const u8) ?i64 {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("if (__self.get(section, option)) |val| return std.fmt.parseInt(i64, val, 10) catch null;\n");
    try self.emitIndent();
    try self.emit("return null;\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn getfloat(__self: *@This(), section: []const u8, option: []const u8) ?f64 {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("if (__self.get(section, option)) |val| return std.fmt.parseFloat(f64, val) catch null;\n");
    try self.emitIndent();
    try self.emit("return null;\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn getboolean(__self: *@This(), section: []const u8, option: []const u8) ?bool {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("if (__self.get(section, option)) |val| {\n");
    self.indent();
    try self.emitIndent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn set(__self: *@This(), section: []const u8, option: []const u8, value: []const u8) void {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("if (__self.sections_map.getPtr(section)) |sec| sec.put(option, value) catch {};\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn add_section(__self: *@This(), section: []const u8) void {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("if (__self.sections_map.get(section) == null) {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("__self.sections_map.put(section, hashmap_helper.StringHashMap([]const u8).init(__global_allocator)) catch {};\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    self.dedent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn options(__self: *@This(), section: []const u8) [][]const u8 {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("var result: std.ArrayList([]const u8) = .{};\n");
    try self.emitIndent();
    try self.emit("if (__self.sections_map.get(section)) |sec| {\n");
    self.indent();
    try self.emitIndent();
