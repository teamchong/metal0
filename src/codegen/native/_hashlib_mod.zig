...
fn genNew(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit(".{ .name = \"sha256\", .digest_size = 32 }"); return; }
    const id = self.nextNameId();
    try self.emitFmt("(__m{d}_new: {{ const name = ", .{id}); try self.genExpr(args[0]);
    try self.emitFmt("; break :__m{d}_new .{{ .name = name, .digest_size = if (std.mem.eql(u8, name, \"md5\")) 16 else if (std.mem.eql(u8, name, \"sha1\")) 20 else if (std.mem.eql(u8, name, \"sha256\")) 32 else if (std.mem.eql(u8, name, \"sha384\")) 48 else if (std.mem.eql(u8, name, \"sha512\")) 64 else 32 }}; }})", .{id});
}
