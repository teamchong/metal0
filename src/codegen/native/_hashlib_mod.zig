...
fn genNew(self: *h.NativeCodegen, args: []ast.Node) h.CodegenError!void {
    ...
    try self.emitFmt("(__m{d}_new: {{ const name = ", .{id});
    try self.genExpr(args[0]);
    try self.emitFmt("; break :__m{d}_new .{{ .name = name, .digest_size = if (std.mem.eql(u8, name, \"md5\")) @as(u8, 16) else if (std.mem.eql(u8, name, \"sha1\")) @as(u8, 20) else if (std.mem.eql(u8, name, \"sha256\")) @as(u8, 32) else if (std.mem.eql(u8, name, \"sha384\")) @as(u8, 48) else if (std.mem.eql(u8, name, \"sha512\")) @as(u8, 64) else @as(u8, 32) }}; }})", .{id});
}
