...
fn genUniform(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("uni", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try c.emit("const _a: f64 = ");
            try c.genExpr(a[0]);
            try c.emit("; const _b: f64 = ");
            try c.genExpr(a[1]);
...
fn genGauss(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("gauss", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try c.emit("const _mu: f64 = ");
            try c.genExpr(a[0]);
            try c.emit("; const _sigma: f64 = ");
            try c.genExpr(a[1]);
...
fn genRandbits(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("bits", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try c.emit("const _k: u6 = @intCast(");
            try c.genExpr(a[0]);
...
pub fn genRandrange(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    if (is_bigint) {
        ...
        if (args.len == 1) {
            ...
            try self.emitFmt("rng_{d}: {{ const __rng_stop_{d} = ", .{ label_id, label_id });
            try self.genExpr(args[0]);
            try self.emitFmt("; {s}const __rng_bits_{d} = try runtime.BigInt.fromInt(__global_allocator, _r.int(u64)); ", .{ prng, label_id });
            try self.emitFmt("break :rng_{d} try __rng_bits_{d}.mod(&__rng_stop_{d}, __global_allocator); }}", .{ label_id, label_id, label_id });
        } else {
            // Two args: randrange(start, stop) - range is [start, stop)
            try self.emitFmt("rng_{d}: {{ const __rng_start_{d} = ", .{ label_id, label_id });
            try self.genExpr(args[0]);
            try self.emitFmt("; const __rng_stop_{d} = ", .{label_id});
            try self.genExpr(args[1]);
            // For BigInt: start + (random_bits % (stop - start))
            try self.emitFmt("; {s}const __rng_bits_{d} = try runtime.BigInt.fromInt(__global_allocator, _r.int(u64)); ", .{ prng, label_id });
            try self.emitFmt("const __rng_range_{d} = try __rng_stop_{d}.sub(&__rng_start_{d}, __global_allocator); ", .{ label_id, label_id, label_id });
            try self.emitFmt("break :rng_{d} try __rng_start_{d}.add(&(try __rng_bits_{d}.mod(&__rng_range_{d}, __global_allocator)), __global_allocator); }}", .{ label_id, label_id, label_id, label_id });
        }
    } else if (args.len == 1) {
        try self.emitFmt("rng_{d}: {{ const __rng_stop_{d}: i64 = @intCast(", .{ label_id, label_id });
        try self.genExpr(args[0]);
        try self.emitFmt("); {s}break :rng_{d} @as(i64, @intCast(_r.int(u64) % @as(u64, @intCast(__rng_stop_{d})))); }}", .{ prng, label_id, label_id });
    } else {
        try self.emitFmt("rng_{d}: {{ const __rng_start_{d}: i64 = @intCast(", .{ label_id, label_id });
        try self.genExpr(args[0]);
        try self.emitFmt("); const __rng_stop_{d}: i64 = @intCast(", .{label_id});
        try self.genExpr(args[1]);
        try self.emitFmt("); {s}break :rng_{d} __rng_start_{d} + @as(i64, @intCast(_r.int(u64) % @as(u64, @intCast(__rng_stop_{d} - __rng_start_{d})))); }}", .{ prng, label_id, label_id, label_id, label_id });
    }
}
...
fn genChoice(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("choice", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[0]);
            try c.emitFmt("; {s}const _len_{d} = if (@TypeOf(__choice_seq_{d}) == runtime.PyValue) __choice_seq_{d}.pyLen() else __choice_seq_{d}.len; const _idx_{d} = _r.int(usize) % _len_{d}; break :{s} if (@TypeOf(__choice_seq_{d}) == runtime.PyValue) __choice_seq_{d}.pyAt(_idx_{d}) else __choice_seq_{d}[_idx_{d}];", .{ prng, id, id, id, id, id, id, label, id, id, id, id, id });
        }
    }.emit);
}
...
fn genChoices(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("choices", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try c.emit("const __choices_seq = ");
            try c.genExpr(a[0]);
            ...
            try c.emitFmt("; {s}var res: std.ArrayListUnmanaged(@TypeOf(__choices_seq[0])) = .{{}}; var i: usize = 0; while (i < k) : (i += 1) res.append(__global_allocator, __choices_seq[_prng.random().int(usize) % __choices_seq.len]) catch continue; break :{s} res.items; ", .{ prng, label });
        }
    }.emit);
}
...
fn genShuffle(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("shuffle", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[0]);
            try c.emitFmt("; {s}const _items_{d} = if (@hasField(@TypeOf(__shuf_seq_{d}), \"items\")) __shuf_seq_{d}.items else __shuf_seq_{d}; _r.shuffle(@TypeOf(_items_{d}[0]), _items_{d}); break :{s}; ", .{ prng, id, id, id, id, id, id, label });
        }
    }.emit);
}
...
fn genGammavariate(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len < 2) {
        try self.emit("@as(f64, 0)");
        return;
    }
    try self.withInlineBlock("gamma", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try c.emit("const _alpha: f64 = ");
            try c.genExpr(a[0]);
            try c.emit("; const _beta: f64 = ");
            try c.genExpr(a[1]);
            try c.emitFmt("; {s}if (_alpha <= 0 or _beta <= 0) break :{s} @as(f64, 0); ", .{ prng, label });
...
fn genBetavariate(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("beta", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.emit("; const _b: f64 = ");
            try c.genExpr(a[1]);
            try c.emitFmt("; {s}const u1 = @as(f64, @floatFromInt(_r.int(u32) + 1)) / @as(f64, @floatFromInt(std.math.maxInt(u32))); ", .{prng});
...
fn genParetovariate(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("pareto", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try c.emit("const _alpha: f64 = ");
            try c.genExpr(a[0]);
            try c.emitFmt("; {s}const u = @as(f64, @floatFromInt(_r.int(u32) + 1)) / @as(f64, @floatFromInt(std.math.maxInt(u32))); ", .{prng});
    ...
    try self.withInlineBlock("tri", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            if (a.len > 2) {
                ...
                try c.emit("(_low + _high) / 2.0");
            }
            try c.emitFmt("; {s}const u = @as(f64, @floatFromInt(_r.int(u32))) / @as(f64, @floatFromInt(std.math.maxInt(u32))); ", .{prng});
