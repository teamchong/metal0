...
pub const Funcs = std.StaticStringMap(h.H).initComptime(.{
    ...
    .{ "chdir", h.wrap("chdir_blk: { const path = ", "; std.posix.chdir(path) catch unreachable; break :chdir_blk {}; }", "{}") },
    .{ "listdir", h.c("runtime.NativeList.init()") },
    .{ "mkdir", h.wrap("mkdir_blk: { const path = ", "; std.fs.cwd().makeDir(path) catch unreachable; break :mkdir_blk {}; }", "{}") },
    .{ "rmdir", h.wrap("rmdir_blk: { const path = ", "; std.fs.cwd().deleteDir(path) catch unreachable; break :rmdir_blk {}; }", "{}") },
    .{ "unlink", h.wrap("unlink_blk: { const path = ", "; std.fs.cwd().deleteFile(path) catch unreachable; break :unlink_blk {}; }", "{}") },
    .{ "rename", h.wrap2("rename_blk: { const src = ", "; const dst = ", "; std.fs.cwd().rename(src, dst) catch unreachable; break :rename_blk {}; }", "{}") },
    ...
    .{ "symlink", h.wrap2("symlink_blk: { const src = ", "; const dst = ", "; std.fs.cwd().symLink(src, dst, .{}) catch unreachable; break :symlink_blk {}; }", "{}") },
