...
fn genGetcwd(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("getcwd", &.{}, struct {
        fn emit(c: *NativeCodegen, label: []const u8, _: []ast.Node) !void {
            try emitFmtConst(c, "var buf: [4096]u8 = undefined; break :{s} std.fs.cwd().realpath(\".\", &buf) catch \".\"; ", .{label});
        }
    }.emit);
}
...
fn genChdir(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("chdir", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; std.posix.chdir(path) catch unreachable; break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genMkdir(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("mkdir", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const path = ");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; std.fs.cwd().makeDir(path) catch unreachable; break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genRmdir(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("rmdir", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const path = ");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; std.fs.cwd().deleteDir(path) catch unreachable; break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genUnlink(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("unlink", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const path = ");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; std.fs.cwd().deleteFile(path) catch unreachable; break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genRename(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("rename", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[1]);
            try emitFmtConst(c, "; std.fs.cwd().rename(src, dst) catch unreachable; break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genStat(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stat", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const path = ");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; const stat = std.fs.cwd().statFile(path) catch break :{s} .{{ .st_size = 0, .st_mode = 0 }}; break :{s} .{{ .st_size = @intCast(stat.size), .st_mode = @intCast(@intFromEnum(stat.kind)) }}; ", .{ label, label });
        }
    }.emit);
}
...
fn genGetenv(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("getenv", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const path = ");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; _ = path; break :{s} if (comptime @import(\"builtin\").os.tag == .windows) @as(?[]const u8, null) else std.posix.getenv(path); ", .{label});
        }
    }.emit);
}
...
fn genKill(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("kill", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[1]);
            try emitFmtConst(c, "; _ = std.c.kill(@intCast(pid), @intCast(sig)); break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genOpen(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("open", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const path = ");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; const file = std.fs.cwd().openFile(path, .{{}}) catch break :{s} @as(i32, -1); break :{s} if (comptime @import(\"builtin\").os.tag == .windows) @as(i32, @truncate(@as(i64, @intFromPtr(file.handle)))) else @intCast(file.handle); ", .{ label, label });
        }
    }.emit);
}
...
fn genClose(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("close", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; std.posix.close(@intCast(fd)); break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genSymlink(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("symlink", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try emitConst(c, "; const dst = ");
            try c.genExpr(a[1]);
            try emitFmtConst(c, "; std.fs.cwd().symLink(src, dst, .{{}}) catch unreachable; break :{s} {{}}; ", .{label});
        }
    }.emit);
}
...
fn genReadlink(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("readlink", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const path = ");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; var buf: [4096]u8 = undefined; break :{s} std.fs.cwd().readLink(path, &buf) catch \"\"; ", .{label});
        }
    }.emit);
}
...
fn genUrandom(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("urandom", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; var buf = __global_allocator.alloc(u8, @intCast(n)) catch break :{s} \"\"; std.crypto.random.bytes(buf); break :{s} buf; ", .{ label, label });
        }
    }.emit);
}
