...
pub const Funcs = std.StaticStringMap(h.H).initComptime(.{
    ...
    .{ "insort_left", h.wrap2("blk: { var _a = ", "; const _x = ", InsortLoop ++ " if (_a.items[_mid] < _x) { _lo = _mid + 1; } else { _hi = _mid; } } _a.insert(__global_allocator, _lo, _x) catch unreachable; break :blk; }", "{}") },
    .{ "insort_right", h.wrap2("blk: { var _a = ", "; const _x = ", InsortLoop ++ " if (_x < _a.items[_mid]) { _hi = _mid; } else { _lo = _mid + 1; } } _a.insert(__global_allocator, _lo, _x) catch unreachable; break :blk; }", "{}") },
    .{ "insort", h.wrap2("blk: { var _a = ", "; const _x = ", InsortLoop ++ " if (_x < _a.items[_mid]) { _hi = _mid; } else { _lo = _mid + 1; } } _a.insert(__global_allocator, _lo, _x) catch unreachable; break :blk; }", "{}") },
});
