...
pub fn genHTTPConnection(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn set_debuglevel(self: *@This(), level: i64) void { _ = level; }\n");
    try self.emitIndent();
    try self.emit("pub fn set_tunnel(self: *@This(), host: []const u8, port: ?u16, headers: anytype) void { _ = host; _ = port; _ = headers; }\n");
    try self.emitIndent();
    try self.emit("pub fn connect(__self: *@This()) void { }\n");
    try self.emitIndent();
    try self.emit("pub fn close(__self: *@This()) void { __self.socket = null; }\n");
    try self.emitIndent();
    try self.emit("pub fn putrequest(self: *@This(), method: []const u8, url: []const u8) void { _ = method; _ = url; }\n");
    try self.emitIndent();
    try self.emit("pub fn putheader(self: *@This(), header: []const u8, value: []const u8) void { _ = header; _ = value; }\n");
    try self.emitIndent();
    try self.emit("pub fn endheaders(self: *@This(), message_body: ?[]const u8) void { _ = message_body; }\n");
    try self.emitIndent();
    try self.emit("pub fn send(self: *@This(), data: []const u8) void { _ = data; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
...
pub fn genHTTPResponse(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn read(self: *@This(), amt: ?usize) []const u8 { _ = amt; return \"\"; }\n");
    try self.emitIndent();
    try self.emit("pub fn readline(self: *@This(), limit: ?usize) []const u8 { _ = limit; return \"\"; }\n");
    try self.emitIndent();
    try self.emit("pub fn readlines(self: *@This(), hint: ?usize) [][]const u8 { _ = hint; return &[_][]const u8{}; }\n");
    try self.emitIndent();
...
pub fn genBaseHTTPRequestHandler(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn send_response(self: *@This(), code: i64, message: ?[]const u8) void { _ = code; _ = message; }\n");
    try self.emitIndent();
    try self.emit("pub fn send_header(self: *@This(), keyword: []const u8, value: []const u8) void { _ = keyword; _ = value; }\n");
    try self.emitIndent();
    try self.emit("pub fn send_error(self: *@This(), code: i64, message: ?[]const u8) void { _ = code; _ = message; }\n");
    try self.emitIndent();
    try self.emit("pub fn end_headers(__self: *@This()) void { }\n");
    try self.emitIndent();
    try self.emit("pub fn log_request(self: *@This(), code: ?i64, size: ?i64) void { _ = code; _ = size; }\n");
    try self.emitIndent();
    try self.emit("pub fn log_error(self: *@This(), format: []const u8) void { _ = format; }\n");
    try self.emitIndent();
    try self.emit("pub fn log_message(self: *@This(), format: []const u8) void { _ = format; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
...
pub fn genSimpleHTTPRequestHandler(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn list_directory(self: *@This(), path: []const u8) ?*anyopaque { _ = path; return null; }\n");
    try self.emitIndent();
    try self.emit("pub fn translate_path(self: *@This(), path: []const u8) []const u8 { return path; }\n");
    try self.emitIndent();
    try self.emit("pub fn guess_type(self: *@This(), path: []const u8) []const u8 { _ = path; return \"application/octet-stream\"; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
...
pub fn genSimpleCookie(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn output(self: *@This(), header: []const u8) []const u8 { _ = header; return \"\"; }\n");
    try self.emitIndent();
    ...
    try self.emit("};\n");
    try self.emitIndent();
    try self.emit("pub fn load(self: *@This(), rawdata: anytype) void { _ = rawdata; }\n");
    try self.emitIndent();
    try self.emit("pub fn output(self: *@This(), header: []const u8, sep: []const u8) []const u8 { _ = header; _ = sep; return \"\"; }\n");
    try self.emitIndent();
    try self.emit("pub fn js_output(__self: *@This()) []const u8 { return \"\"; }\n");
    self.dedent();
    try self.emitIndent();
...
pub fn genHTTPResponseType(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn getheader(self: *@This(), name: []const u8) ?[]const u8 { _ = name; return null; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("};");
}
