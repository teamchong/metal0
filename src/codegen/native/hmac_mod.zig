...
fn genDigest(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("hmac_digest", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try emitConst(c, "; const _msg = ");
            try c.genExpr(a[1]);
            try emitFmtConst(c, "; var _hmac = std.crypto.auth.hmac.sha2.HmacSha256.init(_key); _hmac.update(_msg); const _result = __global_allocator.alloc(u8, 32) catch break :{s} \"\"; _hmac.final(_result[0..32]); break :{s} _result", .{ label, label });
        }
    }.emit);
}
