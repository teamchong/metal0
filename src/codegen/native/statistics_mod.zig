...
pub fn genNormalDist(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn mean(self: @This()) f64 { return __self.mu; }\n");
    try self.emitIndent();
    try self.emit("pub fn median(self: @This()) f64 { return __self.mu; }\n");
    try self.emitIndent();
    try self.emit("pub fn mode(self: @This()) f64 { return __self.mu; }\n");
    try self.emitIndent();
    try self.emit("pub fn stdev(self: @This()) f64 { return __self.sigma; }\n");
    try self.emitIndent();
    try self.emit("pub fn variance(self: @This()) f64 { return __self.sigma * __self.sigma; }\n");
    try self.emitIndent();
    try self.emit("pub fn pdf(self: @This(), x: f64) f64 {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("const z = (x - __self.mu) / __self.sigma;\n");
    try self.emitIndent();
    try self.emit("return @exp(-0.5 * z * z) / (__self.sigma * @sqrt(2.0 * std.math.pi));\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn cdf(self: @This(), x: f64) f64 { _ = x; return 0.5; }\n");
    try self.emitIndent();
    try self.emit("pub fn inv_cdf(self: @This(), p: f64) f64 { _ = p; return 0.0; }\n");
    try self.emitIndent();
    try self.emit("pub fn overlap(self: @This(), other: @This()) f64 { _ = other; return 0.0; }\n");
    try self.emitIndent();
    try self.emit("pub fn samples(self: @This(), n: usize) []f64 { _ = n; return &[_]f64{}; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
