...
fn emitStats(self: *NativeCodegen, args: []ast.Node, comptime label_suffix: []const u8, comptime default: []const u8, comptime body: []const u8) CodegenError!void {
    ...
    const label = try self.emitInlineBlockStart(label_suffix);
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} " ++ default ++ "; " ++ body ++ " ", .{label});
...
fn emitVar(self: *NativeCodegen, args: []ast.Node, comptime label_suffix: []const u8, comptime min_len: []const u8, comptime divisor: []const u8, comptime is_sqrt: bool) CodegenError!void {
    ...
    const label = try self.emitInlineBlockStart(label_suffix);
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len < " ++ min_len ++ ") break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| _sum += @as(f64, @floatFromInt(v)); const _mean = _sum / @as(f64, @floatFromInt(_data.len)); var _sq_sum: f64 = 0.0; for (_data) |v| {{ const d = @as(f64, @floatFromInt(v)) - _mean; _sq_sum += d * d; }} break :{s}", .{ label, label });
...
fn genMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit("@as(f64, 0.0)"); return; }
    const label = try self.emitInlineBlockStart("stats_mean");
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| _sum += @as(f64, @floatFromInt(v)); break :{s} _sum / @as(f64, @floatFromInt(_data.len)); ", .{ label, label });
    try self.emitInlineBlockEnd();
}
fn genGeometricMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit("@as(f64, 0.0)"); return; }
    const label = try self.emitInlineBlockStart("stats_gmean");
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} @as(f64, 0.0); var _prod: f64 = 1.0; for (_data) |v| _prod *= @as(f64, @floatFromInt(v)); break :{s} std.math.pow(f64, _prod, 1.0 / @as(f64, @floatFromInt(_data.len))); ", .{ label, label });
    try self.emitInlineBlockEnd();
}
fn genHarmonicMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit("@as(f64, 0.0)"); return; }
    const label = try self.emitInlineBlockStart("stats_hmean");
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| {{ const fv = @as(f64, @floatFromInt(v)); if (fv != 0) _sum += 1.0 / fv; }} break :{s} if (_sum != 0) @as(f64, @floatFromInt(_data.len)) / _sum else 0.0; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
fn genMedian(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit("@as(f64, 0.0)"); return; }
    const label = try self.emitInlineBlockStart("stats_median");
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} @as(f64, 0.0); const _sorted = __global_allocator.alloc(@TypeOf(_data[0]), _data.len) catch break :{s} @as(f64, 0.0); @memcpy(_sorted, _data); std.mem.sort(i64, _sorted, {{}}, struct {{ fn cmp(_: void, a: i64, b: i64) bool {{ return a < b; }} }}.cmp); const _mid = _sorted.len / 2; break :{s} if (_sorted.len % 2 == 0) (@as(f64, @floatFromInt(_sorted[_mid - 1])) + @as(f64, @floatFromInt(_sorted[_mid]))) / 2.0 else @as(f64, @floatFromInt(_sorted[_mid])); ", .{ label, label, label });
    try self.emitInlineBlockEnd();
}
fn genMedianLow(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit("@as(i64, 0)"); return; }
    const label = try self.emitInlineBlockStart("stats_median_low");
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} @as(i64, 0); var _sorted = __global_allocator.alloc(@TypeOf(_data[0]), _data.len) catch break :{s} @as(i64, 0); @memcpy(_sorted, _data); std.mem.sort(i64, _sorted, {{}}, struct {{ fn cmp(_: void, a: i64, b: i64) bool {{ return a < b; }} }}.cmp); break :{s} _sorted[(_sorted.len - 1) / 2]; ", .{ label, label, label });
    try self.emitInlineBlockEnd();
}
fn genMedianHigh(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit("@as(i64, 0)"); return; }
    const label = try self.emitInlineBlockStart("stats_median_high");
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} @as(i64, 0); var _sorted = __global_allocator.alloc(@TypeOf(_data[0]), _data.len) catch break :{s} @as(i64, 0); @memcpy(_sorted, _data); std.mem.sort(i64, _sorted, {{}}, struct {{ fn cmp(_: void, a: i64, b: i64) bool {{ return a < b; }} }}.cmp); break :{s} _sorted[_sorted.len / 2]; ", .{ label, label, label });
    try self.emitInlineBlockEnd();
}
fn genMode(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) { try self.emit("@as(i64, 0)"); return; }
    const label = try self.emitInlineBlockStart("stats_mode");
    try self.emit("const _data = &"); try self.genExpr(args[0]);
    try self.emitFmt("; if (_data.len == 0) break :{s} @as(@TypeOf(_data[0]), undefined); break :{s} _data[0]; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
