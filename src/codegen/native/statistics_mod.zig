...
fn emitStats(self: *NativeCodegen, args: []ast.Node, comptime label_suffix: []const u8, comptime default: []const u8, comptime body: []const u8) CodegenError!void {
    ...
    const Ctx = struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; if (_data.len == 0) break :{s} " ++ default ++ "; " ++ body ++ " ", .{label});
        }
    };
...
fn genMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_mean", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const _data = &");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; if (_data.len == 0) break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| _sum += @as(f64, @floatFromInt(v)); break :{s} _sum / @as(f64, @floatFromInt(_data.len)); ", .{ label, label });
        }
    }.emit);
}
fn genGeometricMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_gmean", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const _data = &");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; if (_data.len == 0) break :{s} @as(f64, 0.0); var _prod: f64 = 1.0; for (_data) |v| _prod *= @as(f64, @floatFromInt(v)); break :{s} std.math.pow(f64, _prod, 1.0 / @as(f64, @floatFromInt(_data.len))); ", .{ label, label });
        }
    }.emit);
}
fn genHarmonicMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_hmean", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const _data = &");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; if (_data.len == 0) break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| {{ const fv = @as(f64, @floatFromInt(v)); if (fv != 0) _sum += 1.0 / fv; }} break :{s} if (_sum != 0) @as(f64, @floatFromInt(_data.len)) / _sum else 0.0; ", .{ label, label });
        }
    }.emit);
}
...
fn genMedianLow(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_median_low", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const _data = &");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; if (_data.len == 0) break :{s} @as(i64, 0); var _sorted = __global_allocator.alloc(@TypeOf(_data[0]), _data.len) catch break :{s} @as(i64, 0); @memcpy(_sorted, _data); std.mem.sort(i64, _sorted, {{}}, struct {{ fn cmp(_: void, a1: i64, b1: i64) bool {{ return a1 < b1; }} }}.cmp); break :{s} _sorted[(_sorted.len - 1) / 2]; ", .{ label, label, label });
        }
    }.emit);
}
fn genMedianHigh(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_median_high", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const _data = &");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; if (_data.len == 0) break :{s} @as(i64, 0); var _sorted = __global_allocator.alloc(@TypeOf(_data[0]), _data.len) catch break :{s} @as(i64, 0); @memcpy(_sorted, _data); std.mem.sort(i64, _sorted, {{}}, struct {{ fn cmp(_: void, a1: i64, b1: i64) bool {{ return a1 < b1; }} }}.cmp); break :{s} _sorted[_sorted.len / 2]; ", .{ label, label, label });
        }
    }.emit);
}
fn genMode(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_mode", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const _data = &");
            try c.genExpr(a[0]);
            try emitFmtConst(c, "; if (_data.len == 0) break :{s} @as(@TypeOf(_data[0]), undefined); break :{s} _data[0]; ", .{ label, label });
        }
    }.emit);
}
