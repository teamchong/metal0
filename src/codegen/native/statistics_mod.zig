...
fn emitStats(self: *NativeCodegen, args: []ast.Node, comptime label_suffix: []const u8, comptime default: []const u8, comptime body: []const u8) CodegenError!void {
    ...
    const Ctx = struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try b.write("const _data = &");
            const output1 = b.getBodyAndClear();
            try c.output.appendSlice(c.allocator, output1);
            try c.genExpr(a[0]);
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len == 0) break :{s} " ++ default ++ "; " ++ body ++ " ", .{label});
...
fn emitVar(self: *NativeCodegen, args: []ast.Node, comptime label_suffix: []const u8, comptime min_len: []const u8, comptime divisor: []const u8, comptime is_sqrt: bool) CodegenError!void {
    ...
    const Ctx = struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len < " ++ min_len ++ ") break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| _sum += @as(f64, @floatFromInt(v)); const _mean = _sum / @as(f64, @floatFromInt(_data.len)); var _sq_sum: f64 = 0.0; for (_data) |v| {{ const d = @as(f64, @floatFromInt(v)) - _mean; _sq_sum += d * d; }} break :{s}", .{ label, label });
                if (is_sqrt) try b2.write(" @sqrt(_sq_sum / @as(f64, @floatFromInt(_data.len" ++ divisor ++ ")));")
                else try b2.write(" _sq_sum / @as(f64, @floatFromInt(_data.len" ++ divisor ++ "));");
                try b2.write(" ");
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
...
fn genMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_mean", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len == 0) break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| _sum += @as(f64, @floatFromInt(v)); break :{s} _sum / @as(f64, @floatFromInt(_data.len)); ", .{ label, label });
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
        }
...
fn genGeometricMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_gmean", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len == 0) break :{s} @as(f64, 0.0); var _prod: f64 = 1.0; for (_data) |v| _prod *= @as(f64, @floatFromInt(v)); break :{s} std.math.pow(f64, _prod, 1.0 / @as(f64, @floatFromInt(_data.len))); ", .{ label, label });
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
        }
...
fn genHarmonicMean(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_hmean", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len == 0) break :{s} @as(f64, 0.0); var _sum: f64 = 0.0; for (_data) |v| {{ const fv = @as(f64, @floatFromInt(v)); if (fv != 0) _sum += 1.0 / fv; }} break :{s} if (_sum != 0) @as(f64, @floatFromInt(_data.len)) / _sum else 0.0; ", .{ label, label });
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
        }
...
fn genMedianLow(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_median_low", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len == 0) break :{s} @as(i64, 0); var _sorted = __global_allocator.alloc(@TypeOf(_data[0]), _data.len) catch break :{s} @as(i64, 0); @memcpy(_sorted, _data); std.mem.sort(i64, _sorted, {{}}, struct {{ fn cmp(_: void, a1: i64, b1: i64) bool {{ return a1 < b1; }} }}.cmp); break :{s} _sorted[(_sorted.len - 1) / 2]; ", .{ label, label, label });
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
        }
...
fn genMedianHigh(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_median_high", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len == 0) break :{s} @as(i64, 0); var _sorted = __global_allocator.alloc(@TypeOf(_data[0]), _data.len) catch break :{s} @as(i64, 0); @memcpy(_sorted, _data); std.mem.sort(i64, _sorted, {{}}, struct {{ fn cmp(_: void, a1: i64, b1: i64) bool {{ return a1 < b1; }} }}.cmp); break :{s} _sorted[_sorted.len / 2]; ", .{ label, label, label });
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
        }
...
fn genMode(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("stats_mode", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            {
                const b2 = try c.getBuilder();
                try b2.writeFmt("; if (_data.len == 0) break :{s} @as(@TypeOf(_data[0]), undefined); break :{s} _data[0]; ", .{ label, label });
                const output2 = b2.getBodyAndClear();
                try c.output.appendSlice(c.allocator, output2);
            }
        }
