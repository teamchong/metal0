...
pub fn genArray(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn append(self: *@This(), x: i64) void { __self.items.append(__global_allocator, x) catch {}; }\n");
    try self.emitIndent();
    try self.emit("pub fn extend(self: *@This(), iterable: anytype) void { for (iterable) |x| __self.append(x); }\n");
    try self.emitIndent();
    try self.emit("pub fn insert(self: *@This(), i: usize, x: i64) void { __self.items.insert(__global_allocator, i, x) catch {}; }\n");
    try self.emitIndent();
    try self.emit("pub fn remove(self: *@This(), x: i64) void { for (__self.items.items, 0..) |v, i| { if (v == x) { _ = __self.items.orderedRemove(i); return; } } }\n");
    try self.emitIndent();
    try self.emit("pub fn pop(__self: *@This()) i64 { return __self.items.pop(); }\n");
    try self.emitIndent();
    try self.emit("pub fn index(self: *@This(), x: i64) ?usize { for (__self.items.items, 0..) |v, i| { if (v == x) return i; } return null; }\n");
    try self.emitIndent();
    try self.emit("pub fn count(self: *@This(), x: i64) usize { var c: usize = 0; for (__self.items.items) |v| { if (v == x) c += 1; } return c; }\n");
    try self.emitIndent();
    ...
    try self.emitIndent();
    try self.emit("pub fn frombytes(self: *@This(), s: []const u8) void { _ = s; }\n");
    try self.emitIndent();
    try self.emit("pub fn fromlist(self: *@This(), list: []i64) void { for (list) |x| __self.append(x); }\n");
    try self.emitIndent();
    ...
    try self.emitIndent();
    try self.emit("pub fn __getitem__(self: *@This(), i: usize) i64 { return __self.items.items[i]; }\n");
    try self.emitIndent();
    try self.emit("pub fn __setitem__(self: *@This(), i: usize, v: i64) void { __self.items.items[i] = v; }\n");
    try self.emitIndent();
