...
fn methodBlock(comptime label: []const u8, comptime body: []const u8) MH {
    return struct {
        fn f(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
            ...
            try emitConst(self,"const _p = ");
            try self.genExpr(obj);
            try emitFmtConst(self, "; " ++ body ++ " ", .{});
...
fn boolCheck(comptime label: []const u8, comptime check: []const u8, comptime fallback: []const u8) MH {
    return struct {
        fn f(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
            ...
            try emitConst(self,"const _p = ");
            try self.genExpr(obj);
            try emitFmtConst(self, "; " ++ check ++ " catch break :{s} " ++ fallback ++ "; break :{s} true; ", .{ lbl, lbl });
            try self.emitInlineBlockEnd();
        }
    }.f;
}
...
fn prop(comptime pre: []const u8, comptime suf: []const u8) MH {
    return struct {
        fn f(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
            _ = args;
            try emitConst(self,pre);
            try self.genExpr(obj);
            try emitConst(self,suf);
        }
    }.f;
}
...
fn voidOp(comptime label: []const u8, comptime op: []const u8) MH {
    return struct {
        fn f(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
            ...
            try emitConst(self,"const _p = ");
            try self.genExpr(obj);
            try emitFmtConst(self, "; " ++ op ++ " catch {{}}; break :{s} {{}}; ", .{lbl});
            try self.emitInlineBlockEnd();
        }
    }.f;
}
...
fn genPathWrite(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitConst(self,"; const _data = ");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "; const _f = std.fs.cwd().createFile(_p, .{{}}) catch break :{s} @as(usize, 0); defer _f.close(); break :{s} _f.write(_data) catch 0; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathRename(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _old = ");
    try self.genExpr(obj);
    try emitConst(self,"; const _new = ");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "; std.fs.cwd().rename(_old, _new) catch {{}}; break :{s} _new; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathReplace(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"; const _new = ");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "; std.fs.cwd().deleteFile(_new) catch {{}}; std.fs.cwd().rename(_old, _new) catch {{}}; break :{s} _new; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathChmod(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitConst(self,"; const _m: std.fs.File.Mode = @intCast(");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "); const _f = std.fs.cwd().openFile(_p, .{{}}) catch break :{s} {{}}; defer _f.close(); _f.chmod(_m) catch {{}}; break :{s} {{}}; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathJoinpath(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _base = ");
    try self.genExpr(obj);
    try emitConst(self,"; const _parts = [_][]const u8{ _base");
    for (args) |arg| {
        try emitConst(self,", ");
        try self.genExpr(arg);
    }
    try emitFmtConst(self, " }}; break :{s} std.fs.path.join(__global_allocator, &_parts) catch _base; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathGlob(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitConst(self,"; const _pattern = ");
    if (args.len > 0) {
        try self.genExpr(args[0]);
    } else {
        try emitConst(self,"\"*\"");
    ...
    try emitFmtConst(self, "{s} _entries; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathExists(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; _ = std.fs.cwd().statFile(_p) catch {{ _ = std.fs.cwd().openDir(_p, .{{}}) catch break :{s} false; break :{s} true; }}; break :{s} true; ", .{ label, label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathIsDir(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; var _d = std.fs.cwd().openDir(_p, .{{}}) catch break :{s} false; _d.close(); break :{s} true; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathIsSymlink(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; const _s = std.fs.cwd().statFile(_p) catch break :{s} false; break :{s} _s.kind == .sym_link; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathIsAbsolute(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; break :{s} std.fs.path.isAbsolute(_p); ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathReadText(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; break :{s} std.fs.cwd().readFileAlloc(__global_allocator, _p, 10 * 1024 * 1024) catch \"\"; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathTouch(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; const _f = std.fs.cwd().createFile(_p, .{{ .exclusive = false }}) catch break :{s} {{}}; _f.close(); break :{s} {{}}; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathStat(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; const _s = std.fs.cwd().statFile(_p) catch break :{s} .{{ .st_size = 0, .st_mode = 0 }}; break :{s} .{{ .st_size = @as(i64, @intCast(_s.size)), .st_mode = @as(u32, @intCast(_s.mode)) }}; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathAbsolute(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; const _cwd = std.process.getCwdAlloc(__global_allocator) catch break :{s} _p; break :{s} std.fs.path.join(__global_allocator, &.{{_cwd, _p}}) catch _p; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathResolve(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; break :{s} std.fs.cwd().realpathAlloc(__global_allocator, _p) catch _p; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathExpanduser(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; if (_p.len > 0 and _p[0] == '~') {{ const _h = if (comptime @import(\"builtin\").os.tag == .windows) \"C:\\\\Users\\\\Public\" else (std.posix.getenv(\"HOME\") orelse \"\"); break :{s} std.fs.path.join(__global_allocator, &.{{_h, _p[1..]}}) catch _p; }} break :{s} _p; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathWithName(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"; const _arg = ");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "; const _d = std.fs.path.dirname(_p) orelse \"\"; break :{s} std.fs.path.join(__global_allocator, &.{{_d, _arg}}) catch _p; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathWithSuffix(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"; const _arg = ");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "; const _ext = std.fs.path.extension(_p); const _stem = if (_ext.len > 0) _p[0.._p.len - _ext.len] else _p; break :{s} std.fmt.allocPrint(__global_allocator, \"{{s}}{{s}}\", .{{_stem, _arg}}) catch _p; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathWithStem(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"; const _arg = ");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "; const _ext = std.fs.path.extension(_p); const _d = std.fs.path.dirname(_p) orelse \"\"; break :{s} std.fmt.allocPrint(__global_allocator, \"{{s}}/{{s}}{{s}}\", .{{_d, _arg, _ext}}) catch _p; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathRelativeTo(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"; const _arg = ");
    try self.genExpr(args[0]);
    try emitFmtConst(self, "; _ = _arg; break :{s} _p; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathStem(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; const _n = std.fs.path.basename(_p); const _e = std.fs.path.extension(_n); break :{s} if (_e.len > 0) _n[0.._n.len - _e.len] else _n; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathSuffixes(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; var _s: std.ArrayList([]const u8) = .{{}}; const _n = std.fs.path.basename(_p); var _i: usize = 0; while (_i < _n.len) : (_i += 1) {{ if (_n[_i] == '.') {{ _s.append(__global_allocator, _n[_i..]) catch continue; break; }} }} break :{s} _s.items; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathParents(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; var _ps: std.ArrayList([]const u8) = .{{}}; var _cur = _p; while (std.fs.path.dirname(_cur)) |_d| {{ _ps.append(__global_allocator, _d) catch break; _cur = _d; }} break :{s} _ps.items; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathParts(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; var _ps: std.ArrayList([]const u8) = .{{}}; var _it = std.mem.splitScalar(u8, _p, '/'); while (_it.next()) |_part| {{ if (_part.len > 0) _ps.append(__global_allocator, _part) catch continue; }} break :{s} _ps.items; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathAnchor(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; break :{s} if (_p.len > 0 and _p[0] == '/') \"/\" else \"\"; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathOpen(self: *NativeCodegen, obj: ast.Node, args: []ast.Node) CodegenError!void {
    ...
    try emitConst(self,"const _p = ");
    try self.genExpr(obj);
    try emitFmtConst(self, "; break :{s} try runtime.PyFile.open(_p, \"r\", __global_allocator); ", .{label});
    try self.emitInlineBlockEnd();
}
