...
/// Uses ZigBuilder for unique block labels (body uses _path which is block-scoped)
fn pathBlock(comptime name: []const u8, comptime body: []const u8, comptime result: []const u8) H {
    return struct {
        fn f(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
            ...
            try self.emit("; " ++ body);
            try self.emitFmt("break :{s} " ++ result ++ "; ", .{label});
...
/// Uses ZigBuilder for unique block labels
fn osSwitch(comptime name: []const u8, comptime win: []const u8, comptime posix: []const u8) H {
    return struct {
        fn f(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
            _ = args;
            const label = try self.emitInlineBlockStart(name);
            try self.emitFmt("const _builtin = @import(\"builtin\"); break :{s} switch (_builtin.os.tag) {{ .windows => " ++ win ++ ", else => " ++ posix ++ " }}; ", .{label});
            try self.emitInlineBlockEnd();
        }
    }.f;
}
...
fn genChmod(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    if (args.len < 2) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("chmod");
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emit("; const _mode: std.fs.File.Mode = @intCast(");
    try self.genExpr(args[1]);
    try self.emitFmt("); const _f = std.fs.cwd().openFile(_path, .{{}}) catch break :{s} {{}}; defer _f.close(); _f.chmod(_mode) catch {{}}; break :{s} {{}}; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genAccess(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("; _ = ");
    try self.genExpr(args[1]);
    try self.emitFmt("; _ = std.fs.cwd().statFile(_p) catch break :{s} false; break :{s} true; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genTruncate(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.genExpr(args[1]);
    try self.emitFmt(")); var _f = std.fs.cwd().openFile(_p, .{{ .mode = .write_only }}) catch break :{s} {{}}; defer _f.close(); _f.setEndPos(_len) catch {{}}; break :{s} {{}}; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genKill(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("const _pid: std.posix.pid_t = @intCast(");
    try self.genExpr(args[0]);
    try self.emit("); const _sig: u6 = @intCast(");
    try self.genExpr(args[1]);
    try self.emitFmt("); _ = std.posix.kill(_pid, _sig) catch {{}}; break :{s} {{}}; ", .{label});
    try self.emitInlineBlockEnd();
}

fn genSystem(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("system");
    ...
    try self.emitFmt("_ = _child.spawn() catch break :{s} @as(i64, -1); ", .{label});
    try self.emitFmt("const _r = _child.wait() catch break :{s} @as(i64, -1); ", .{label});
    try self.emitFmt("break :{s} @as(i64, @intCast(_r.Exited)); ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genRead(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len < 2) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("read");
    try self.emit("const _fd = @as(std.posix.fd_t, @intCast(");
    try self.genExpr(args[0]);
    try self.emit(")); const _n = @as(usize, @intCast(");
    try self.genExpr(args[1]);
    try self.emitFmt(")); var _buf = try __global_allocator.alloc(u8, _n); const _read = std.posix.read(_fd, _buf) catch 0; break :{s} _buf[0.._read]; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genOpen(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.genExpr(args[1]);
    try self.emitFmt(")); _ = _flags; const _f = std.fs.cwd().openFile(_p, .{{}}) catch break :{s} @as(i64, -1); break :{s} if (comptime @import(\"builtin\").os.tag == .windows) @as(i64, @intFromPtr(_f.handle)) else @as(i64, @intCast(_f.handle)); ", .{ label, label });
    try self.emitInlineBlockEnd();
}

fn genPipe(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    _ = args;
    const label = try self.emitInlineBlockStart("pipe");
    try self.emitFmt("const _p = std.posix.pipe() catch break :{s} .{{ @as(i64, -1), @as(i64, -1) }}; break :{s} .{{ @as(i64, @intCast(_p[0])), @as(i64, @intCast(_p[1])) }}; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genFdopen(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    const label = try self.emitInlineBlockStart("fdopen");
    try self.emit("const _fd_int: i64 = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; const _builtin = @import(\"builtin\"); const _handle = if (comptime _builtin.os.tag == .windows) @as(*anyopaque, @ptrFromInt(@as(usize, @intCast(_fd_int)))) else @as(std.posix.fd_t, @intCast(_fd_int)); break :{s} std.fs.File{{ .handle = _handle }}; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genUrandom(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("const _n = @as(usize, @intCast(");
    try self.genExpr(args[0]);
    try self.emitFmt(")); var _buf = try __global_allocator.alloc(u8, _n); std.crypto.random.bytes(_buf); break :{s} _buf; ", .{label});
    try self.emitInlineBlockEnd();
}

fn genWalk(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("walk");
    try self.emit("const _root = ");
    try self.genExpr(args[0]);
...
fn genScandir(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    const label = try self.emitInlineBlockStart("scandir");
    if (args.len >= 1) {
...
fn genSymlink(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.genExpr(args[1]);
    try self.emitFmt("; std.fs.cwd().symLink(_src, _dst, .{{}}) catch {{}}; break :{s} {{}}; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genReadlink(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("const _p = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; var _buf: [4096]u8 = undefined; break :{s} std.fs.cwd().readLink(_p, &_buf) catch \"\"; ", .{label});
    try self.emitInlineBlockEnd();
}
...
fn genPathExists(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; _ = std.fs.cwd().statFile(_path) catch {{ _ = std.fs.cwd().openDir(_path, .{{}}) catch break :{s} false; break :{s} true; }}; break :{s} true; ", .{ label, label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathIsdir(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; var _dir = std.fs.cwd().openDir(_path, .{{}}) catch break :{s} false; _dir.close(); break :{s} true; ", .{ label, label });
    try self.emitInlineBlockEnd();
}

fn genPathIsfile(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("path_isfile");
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; const _stat = std.fs.cwd().statFile(_path) catch break :{s} false; _ = _stat; break :{s} true; ", .{ label, label });
    try self.emitInlineBlockEnd();
}

fn genPathAbspath(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("path_abspath");
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; const _cwd = std.process.getCwdAlloc(__global_allocator) catch break :{s} _path; break :{s} std.fs.path.join(__global_allocator, &[_][]const u8{{_cwd, _path}}) catch _path; ", .{ label, label });
    try self.emitInlineBlockEnd();
}
...
fn genPathJoin(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitFmt(" }}; break :{s} std.fs.path.join(__global_allocator, &_paths) catch \"\"; ", .{label});
    try self.emitInlineBlockEnd();
}

fn genPathSplit(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("path_split");
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; const _dirname = std.fs.path.dirname(_path) orelse \"\"; const _basename = std.fs.path.basename(_path); break :{s} .{{ .@\"0\" = _dirname, .@\"1\" = _basename }}; ", .{label});
    try self.emitInlineBlockEnd();
}

fn genPathSplitext(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("path_splitext");
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; const _ext = std.fs.path.extension(_path); const _root = if (_ext.len > 0) _path[0.._path.len - _ext.len] else _path; break :{s} .{{ .@\"0\" = _root, .@\"1\" = _ext }}; ", .{label});
    try self.emitInlineBlockEnd();
}

fn genPathGetsize(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len == 0) return error.UnsupportedSyntax;
    const label = try self.emitInlineBlockStart("path_getsize");
    try self.emit("const _path = ");
    try self.genExpr(args[0]);
    try self.emitFmt("; const _stat = std.fs.cwd().statFile(_path) catch break :{s} @as(i64, 0); break :{s} @as(i64, @intCast(_stat.size)); ", .{ label, label });
    try self.emitInlineBlockEnd();
}
