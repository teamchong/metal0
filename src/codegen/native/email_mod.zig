...
pub const EmailMimeMultipartFuncs = std.StaticStringMap(ModuleHandler).initComptime(.{ .{ "MIMEMultipart", genMIMEMulti } });
pub const EmailMimeBaseFuncs = std.StaticStringMap(ModuleHandler).initComptime(.{ .{ "MIMEBase", genMIMEBase }, .{ "MIMEApplication", genMIMEApp }, .{ "MIMEImage", genMIMEApp }, .{ "MIMEAudio", genMIMEApp } });
pub const EmailUtilsFuncs = std.StaticStringMap(ModuleHandler).initComptime(.{ .{ "formataddr", genEmptyStr }, .{ "parseaddr", genParseTup }, .{ "formatdate", genEmptyStr }, .{ "make_msgid", genMsgId } });
