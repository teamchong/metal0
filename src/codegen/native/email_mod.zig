...
pub fn genEmailMessage(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn set_content(self: *@This(), content: []const u8) void { __self.body = content; }\n");
    try self.emitIndent();
    ...
    try self.emitIndent();
    try self.emit("pub fn get(self: *@This(), name: []const u8) ?[]const u8 { return __self.headers.get(name); }\n");
    try self.emitIndent();
    try self.emit("pub fn set(self: *@This(), name: []const u8, value: []const u8) void { __self.headers.put(name, value) catch {}; }\n");
    try self.emitIndent();
    try self.emit("pub fn add_header(self: *@This(), name: []const u8, value: []const u8) void { __self.set(name, value); }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}.init()");
}
...
pub fn genMIMEMultipart(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("headers: hashmap_helper.StringHashMap([]const u8) = .{},\n");
    try self.emitIndent();
    try self.emit("pub fn attach(self: *@This(), part: anytype) void { __self.parts.append(__global_allocator, part.as_string()) catch {}; }\n");
    try self.emitIndent();
...
pub fn genMIMEBase(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn set_payload(self: *@This(), data: []const u8) void { __self.payload = data; }\n");
    try self.emitIndent();
    try self.emit("pub fn get_payload(__self: *@This()) []const u8 { return __self.payload; }\n");
    try self.emitIndent();
    try self.emit("pub fn add_header(self: *@This(), name: []const u8, value: []const u8) void { _ = name; _ = value; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
