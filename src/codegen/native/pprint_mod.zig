...
pub fn genPrettyPrinter(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn pprint(self: @This(), object: anytype) void { std.debug.print(\"{any}\\n\", .{object}); }\n");
    try self.emitIndent();
    try self.emit("pub fn pformat(self: @This(), object: anytype) []const u8 { _ = object; return \"\"; }\n");
    try self.emitIndent();
    try self.emit("pub fn isreadable(self: @This(), object: anytype) bool { _ = object; return true; }\n");
    try self.emitIndent();
    try self.emit("pub fn isrecursive(self: @This(), object: anytype) bool { _ = object; return false; }\n");
    try self.emitIndent();
    try self.emit("pub fn format(self: @This(), object: anytype) []const u8 { _ = object; return \"\"; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
