...
fn genEscape(self: *h.NativeCodegen, args: []ast.Node) h.CodegenError!void {
    ...
    const id = try h.emitUniqueBlockStart(self, "html_escape");
    try self.emit("const _s = ");
    try self.genExpr(args[0]);
    try self.emit("; var _result: std.ArrayList(u8) = .{}; for (_s) |c| { switch (c) { '&' => _result.appendSlice(__global_allocator, \"&amp;\") catch unreachable, '<' => _result.appendSlice(__global_allocator, \"&lt;\") catch unreachable, '>' => _result.appendSlice(__global_allocator, \"&gt;\") catch unreachable, '\"' => _result.appendSlice(__global_allocator, \"&quot;\") catch unreachable, '\\'' => _result.appendSlice(__global_allocator, \"&#x27;\") catch unreachable, else => _result.append(__global_allocator, c) catch unreachable, } }");
    try h.emitBlockBreak(self, "html_escape", id);
