...
pub const Funcs = std.StaticStringMap(ModuleHandler).initComptime(.{
    ...
    .{ "ZERO_OR_MORE", genZERO_OR_MORE },
    .{ "ONE_OR_MORE", genONE_OR_MORE },
});
...
pub fn genArgumentParser(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit(".parsed = .{},\n");
    try self.emitIndent();
    try self.emit(".positional_args = .{},\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("};\n");
    self.dedent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn add_argument(__self: *@This(), name: []const u8) void {\n");
    self.indent();
    try self.emitIndent();
    ...
    self.indent();
    try self.emitIndent();
    try self.emit("const args = std.process.argsAlloc(__global_allocator) catch return __self;\n");
    try self.emitIndent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn get(__self: *@This(), name: []const u8) ?[]const u8 {\n");
    self.indent();
    try self.emitIndent();
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn get_positional(__self: *@This(), index: usize) ?[]const u8 {\n");
    self.indent();
    try self.emitIndent();
...
pub fn genNamespace(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn set(__self: *@This(), key: []const u8, val: []const u8) void { __self.data.put(key, val) catch {}; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}.init()");
}
