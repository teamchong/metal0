...
fn genLeapdays(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("leapdays", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const y1 = ");
            try c.genExpr(a[0]);
            try emitConst(c, "; const y2 = ");
            try c.genExpr(a[1]);
            try emitFmtConst(c, "; break :{s} @divFloor(y2 - 1, 4) - @divFloor(y1 - 1, 4) - (@divFloor(y2 - 1, 100) - @divFloor(y1 - 1, 100)) + (@divFloor(y2 - 1, 400) - @divFloor(y1 - 1, 400)); ", .{label});
        }
    }.emit);
}
...
fn genWeekday(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.withInlineBlock("weekday", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            try emitConst(c, "const __year = @as(i32, @intCast(");
            try c.genExpr(a[0]);
            try emitConst(c, ")); const __month = @as(i32, @intCast(");
            try c.genExpr(a[1]);
            try emitConst(c, ")); const __day = @as(i32, @intCast(");
            try c.genExpr(a[2]);
            try emitConst(c, "));" ++ zeller);
            try emitFmtConst(c, " const __h = @rem(@as(i32, __day + @divFloor(13 * (__m + 1), 5) + __k + @divFloor(__k, 4) + @divFloor(__j, 4) - 2 * __j + 700), 7); break :{s} @rem(__h + 5, 7); ", .{label});
        }
    }.emit);
}
...
fn genMonthrange(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len < 2) {
        try emitConst(self, ".{ @as(i32, 0), @as(i32, 30) }");
        return;
    }
    try self.withInlineBlock("monthrange", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, a: []ast.Node) !void {
            ...
            try c.genExpr(a[1]);
            try emitConst(c, "));");
            try emitConst(c, " const __days_in_month = [_]i32{ 0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };");
            try emitConst(c, " const __is_leap = (@rem(__year, 4) == 0 and @rem(__year, 100) != 0) or @rem(__year, 400) == 0;");
            try emitConst(c, " const __ndays = if (__month == 2 and __is_leap) 29 else __days_in_month[@intCast(__month)];" ++ zeller);
            try emitConst(c, " const __h = @rem(@as(i32, 1 + @divFloor(13 * (__m + 1), 5) + __k + @divFloor(__k, 4) + @divFloor(__j, 4) - 2 * __j + 700), 7);");
            try emitFmtConst(c, " break :{s} .{{ @rem(__h + 5, 7), __ndays }}; ", .{label});
        }
    }.emit);
}
