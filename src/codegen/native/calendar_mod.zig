...
fn genLeapdays(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    const label = try self.emitInlineBlockStart("leapdays");
    try self.emit("const y1 = ");
    try self.genExpr(args[0]);
    try self.emit("; const y2 = ");
    try self.genExpr(args[1]);
    try self.emitFmt("; break :{s} @divFloor(y2 - 1, 4) - @divFloor(y1 - 1, 4) - (@divFloor(y2 - 1, 100) - @divFloor(y1 - 1, 100)) + (@divFloor(y2 - 1, 400) - @divFloor(y1 - 1, 400)); ", .{label});
...
fn genWeekday(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    const label = try self.emitInlineBlockStart("weekday");
    ...
    try self.emitFmt(" const __h = @rem(@as(i32, __day + @divFloor(13 * (__m + 1), 5) + __k + @divFloor(__k, 4) + @divFloor(__j, 4) - 2 * __j + 700), 7); break :{s} @rem(__h + 5, 7); ", .{label});
    try self.emitInlineBlockEnd();
}

fn genMonthrange(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len < 2) { try self.emit(".{ @as(i32, 0), @as(i32, 30) }"); return; }
    const label = try self.emitInlineBlockStart("monthrange");
    try self.emit("const __year = @as(i32, @intCast("); try self.genExpr(args[0]); try self.emit("));");
    try self.emit(" const __month = @as(i32, @intCast("); try self.genExpr(args[1]); try self.emit("));");
    ...
    try self.emitFmt(" break :{s} .{{ @rem(__h + 5, 7), __ndays }}; ", .{label});
    try self.emitInlineBlockEnd();
}
