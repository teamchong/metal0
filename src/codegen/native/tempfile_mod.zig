...
fn genMktemp(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    try self.withInlineBlock("mkt", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, _: []ast.Node) !void {
            try c.emit(prng_init ++ "const _name = std.fmt.bufPrint(&_buf, \"/tmp/tmp");
            try c.emit("{x:0>8}");
            try c.emitFmt("\", .{{_rand.int(u32)}}) catch break :{s} \"/tmp/tmpXXXXXXXX\"; break :{s} _name; ", .{ label, label });
        }
    }.emit);
}
...
fn genMkdtemp(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    try self.withInlineBlock("mkd", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, _: []ast.Node) !void {
            try c.emit(prng_init ++ "const _name = std.fmt.bufPrint(&_buf, \"/tmp/tmp");
            try c.emit("{x:0>8}");
            try c.emitFmt("\", .{{_rand.int(u32)}}) catch break :{s} \"/tmp/tmpXXXXXXXX\"; std.fs.makeDirAbsolute(_name) catch unreachable; break :{s} _name; ", .{ label, label });
        }
    }.emit);
}
...
fn genMkstemp(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    try self.withInlineBlock("mks", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, _: []ast.Node) !void {
            try c.emit(prng_init ++ "const _name = std.fmt.bufPrint(&_buf, \"/tmp/tmp");
            try c.emit("{x:0>8}");
            try c.emitFmt("\", .{{_rand.int(u32)}}) catch break :{s} .{{ @as(i64, -1), \"\" }}; const _file = std.fs.createFileAbsolute(_name, .{{}}) catch break :{s} .{{ @as(i64, -1), _name }}; break :{s} .{{ if (comptime @import(\"builtin\").os.tag == .windows) @as(i64, @intFromPtr(_file.handle)) else @as(i64, @intCast(_file.handle)), _name }}; ", .{ label, label, label });
        }
    }.emit);
}
...
fn genNamedTempFile(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    try self.withInlineBlock("ntf", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, _: []ast.Node) !void {
            try c.emit(prng_init ++ "const _name = std.fmt.bufPrint(&_buf, \"/tmp/tmp");
            try c.emit("{x:0>8}");
            try c.emitFmt("\", .{{_rand.int(u32)}}) catch break :{s} struct {{ name: []const u8, file: ?std.fs.File }}{{ .name = \"\", .file = null }}; const _file = std.fs.createFileAbsolute(_name, .{{}}) catch break :{s} struct {{ name: []const u8, file: ?std.fs.File }}{{ .name = _name, .file = null }}; break :{s} struct {{ name: []const u8, file: ?std.fs.File }}{{ .name = _name, .file = _file }}; ", .{ label, label, label });
        }
    }.emit);
}
...
fn genTempDir(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    try self.withInlineBlock("td", args, struct {
        fn emit(c: *NativeCodegen, label: []const u8, _: []ast.Node) !void {
            try c.emit(prng_init ++ "const _name = std.fmt.bufPrint(&_buf, \"/tmp/tmpdir");
            try c.emit("{x:0>8}");
            try c.emitFmt("\", .{{_rand.int(u32)}}) catch break :{s} struct {{ name: []const u8 }}{{ .name = \"\" }}; std.fs.makeDirAbsolute(_name) catch unreachable; break :{s} struct {{ name: []const u8, pub fn cleanup(__self: *@This()) void {{ std.fs.deleteTreeAbsolute(__self.name) catch unreachable; }} pub fn __enter__(__self: *@This()) []const u8 {{ return __self.name; }} pub fn __exit__(__self: *@This(), _: anytype) void {{ __self.cleanup(); }} }}{{ .name = _name }}; ", .{ label, label });
        }
    }.emit);
}
