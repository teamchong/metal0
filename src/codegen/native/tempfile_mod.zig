...
pub fn genSpooledTemporaryFile(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn read(self: *@This()) []const u8 { return self.buffer.items; }\n");
    try self.emitIndent();
    try self.emit("pub fn seek(self: *@This(), pos: usize) void { self.pos = pos; }\n");
    try self.emitIndent();
    try self.emit("pub fn tell(self: *@This()) usize { return self.pos; }\n");
    try self.emitIndent();
    try self.emit("pub fn close(self: *@This()) void { self.buffer.deinit(allocator); }\n");
    self.dedent();
    try self.emitIndent();
...
pub fn genTemporaryDirectory(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn cleanup(self: *@This()) void {\n");
    self.indent();
    try self.emitIndent();
    try self.emit("std.fs.deleteTreeAbsolute(self.name) catch {};\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}\n");
    try self.emitIndent();
    try self.emit("pub fn __enter__(self: *@This()) []const u8 { return self.name; }\n");
    try self.emitIndent();
    try self.emit("pub fn __exit__(self: *@This(), _: anytype) void { self.cleanup(); }\n");
    self.dedent();
    try self.emitIndent();
