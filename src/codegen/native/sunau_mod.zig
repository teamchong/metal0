...
pub const Funcs = std.StaticStringMap(h.H).initComptime(.{
    .{ "open", h.structBlk("sunau", ".file = __v, .mode = \"rb\"", ".{ .file = @as(?*anyopaque, null), .mode = \"rb\" }") },
    .{ "Au_read", h.c(".{ .nchannels = @as(i32, 0), .sampwidth = @as(i32, 0), .framerate = @as(i32, 0), .nframes = @as(i32, 0), .comptype = \"NONE\", .compname = \"not compressed\" }") },
