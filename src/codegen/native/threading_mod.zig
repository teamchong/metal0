...
pub fn genThread(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn setName(self: *@This(), n: []const u8) void { __self.name = n; }\n");
    try self.emitIndent();
    try self.emit("pub fn isDaemon(__self: *@This()) bool { return __self.daemon; }\n");
    try self.emitIndent();
    try self.emit("pub fn setDaemon(self: *@This(), d: bool) void { __self.daemon = d; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
...
pub fn genLock(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn __enter__(__self: *@This()) *@This() { __self.acquire(); return self; }\n");
    try self.emitIndent();
    try self.emit("pub fn __exit__(self: *@This(), _: anytype) void { __self.release(); }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{}");
}
