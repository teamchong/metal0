...
pub const Funcs = std.StaticStringMap(ModuleHandler).initComptime(.{
    ...
    .{ "ElementTree", genElementTree },
    .{ "Comment", genComment },
    .{ "ProcessingInstruction", genProcessingInstruction },
    .{ "QName", genQName },
    .{ "indent", genIndent },
    .{ "dump", genDump },
    .{ "iselement", genIselement },
});
...
pub fn genParse(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit(";\n");
    try self.emitIndent();
    try self.emit("const file = std.fs.cwd().openFile(_source, .{}) catch break :xml_parse_blk struct { root: ?*Element = null, pub fn getroot(__self: *@This()) ?*Element { return __self.root; } }{};\n");
    try self.emitIndent();
    try self.emit("defer file.close();\n");
    try self.emitIndent();
    try self.emit("const content = file.readToEndAlloc(__global_allocator, 10 * 1024 * 1024) catch break :xml_parse_blk struct { root: ?*Element = null, pub fn getroot(__self: *@This()) ?*Element { return __self.root; } }{};\n");
    try self.emitIndent();
    try self.emit("_ = content;\n");
    try self.emitIndent();
    try self.emit("break :xml_parse_blk struct { root: ?*Element = null, pub fn getroot(__self: *@This()) ?*Element { return __self.root; } }{};\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}");
}
...
pub fn genTostring(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("_result.appendSlice(__global_allocator, _elem.tag) catch {};\n");
    try self.emitIndent();
    try self.emit("_result.appendSlice(__global_allocator, \">\") catch {};\n");
    try self.emitIndent();
    try self.emit("_result.appendSlice(__global_allocator, _elem.text) catch {};\n");
    try self.emitIndent();
    try self.emit("_result.appendSlice(__global_allocator, \"</\") catch {};\n");
    try self.emitIndent();
    try self.emit("_result.appendSlice(__global_allocator, _elem.tag) catch {};\n");
...
pub fn genSubElement(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("_parent.children.append(__global_allocator, &_child) catch {};\n");
    try self.emitIndent();
...
pub fn genElementStruct(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("attrib: hashmap_helper.StringHashMap([]const u8) = .{},\n");
    try self.emitIndent();
    ...
    try self.emitIndent();
    try self.emit("for (__self.children.items) |child| if (std.mem.eql(u8, child.tag, path)) result.append(__global_allocator, child) catch {};\n");
    try self.emitIndent();
