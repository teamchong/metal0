...
pub fn genEnum(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn __str__(self: @This()) []const u8 { return __self.name; }\n");
    try self.emitIndent();
    try self.emit("pub fn __repr__(self: @This()) []const u8 { return __self.name; }\n");
    self.dedent();
    try self.emitIndent();
...
pub fn genStrEnum(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emitIndent();
    try self.emit("pub fn __str__(self: @This()) []const u8 { return __self.value; }\n");
    try self.emitIndent();
    try self.emit("pub fn __repr__(self: @This()) []const u8 { return __self.name; }\n");
    self.dedent();
    try self.emitIndent();
...
pub fn genFlag(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try self.emit("value: i64,\n");
    try self.emitIndent();
    try self.emit("pub fn __or__(self: @This(), other: @This()) @This() { return @This(){ .name = __self.name, .value = __self.value | other.value }; }\n");
    try self.emitIndent();
    try self.emit("pub fn __and__(self: @This(), other: @This()) @This() { return @This(){ .name = __self.name, .value = __self.value & other.value }; }\n");
    try self.emitIndent();
    try self.emit("pub fn __xor__(self: @This(), other: @This()) @This() { return @This(){ .name = __self.name, .value = __self.value ^ other.value }; }\n");
    try self.emitIndent();
    try self.emit("pub fn __invert__(self: @This()) @This() { return @This(){ .name = __self.name, .value = ~__self.value }; }\n");
    self.dedent();
    try self.emitIndent();
    try self.emit("}{ .name = \"\", .value = 0 }");
}
