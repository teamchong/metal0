...
fn handleEncode(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    ...
    try emitFmtConst(self, "(__tok_enc_{d}: {{ ", .{id});
    ...
    try emitFmtConst(self, "const __enc_tokens_{d} = try runtime.tokenizer.encode(__global_allocator, ", .{id});
    if (args.len > 0) {
    ...
    try emitConst(self, "); ");
    try emitFmtConst(self, "const __enc_list_{d} = try runtime.PyList.create(__global_allocator); ", .{id});
    try emitFmtConst(self, "for (__enc_tokens_{d}) |__enc_tok_{d}| {{ try runtime.PyList.append(__enc_list_{d}, try runtime.PyInt.create(__global_allocator, @intCast(__enc_tok_{d}))); }} ", .{ id, id, id, id });
    try emitFmtConst(self, "break :__tok_enc_{d} __enc_list_{d}; }})", .{ id, id });
}
...
fn handleDecode(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    const id = self.nextNameId();
    try emitFmtConst(self, "(__tok_dec_{d}: {{ const __dec_list_{d} = ", .{ id, id });
    if (args.len > 0) {
        try self.genExpr(args[0]);
    }
    try emitConst(self, "; ");
    // Convert PyList to []u32
    try emitFmtConst(self, "var __dec_tokens_{d} = try __global_allocator.alloc(u32, runtime.PyList.len(__dec_list_{d})); ", .{ id, id });
    try emitFmtConst(self, "var __dec_i_{d}: usize = 0; ", .{id});
    try emitFmtConst(self, "while (__dec_i_{d} < __dec_tokens_{d}.len) : (__dec_i_{d} += 1) {{ ", .{ id, id, id });
    try emitFmtConst(self, "const __dec_item_{d} = try runtime.PyList.getItem(__dec_list_{d}, __dec_i_{d}); ", .{ id, id, id });
    try emitFmtConst(self, "__dec_tokens_{d}[__dec_i_{d}] = @intCast(runtime.PyInt.getValue(__dec_item_{d})); ", .{ id, id, id });
    try emitConst(self, "} ");
    try emitFmtConst(self, "break :__tok_dec_{d} try runtime.tokenizer.decode(__global_allocator, __dec_tokens_{d}); }})", .{ id, id });
}
