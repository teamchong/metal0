...
fn genExpanduser(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len > 0) {
        ...
        try self.emit("_expanduser std.fmt.allocPrint(__global_allocator, \"{s}{s}\", .{ home, path[1..] }) catch path; } break :__m");
        try self.emitFmt("{d}_expanduser path; }}", .{id});
    } else try self.emit("\"\"");
}
...
fn genIsabs(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len > 0) {
        ...
        try self.genExpr(args[0]);
        try self.emitFmt("; break :__m{d}_isabs (path.len > 0 and path[0] == '/') or (path.len > 2 and path[1] == ':'); }}", .{id});
    } else try self.emit("false");
}
...
fn genNormcase(self: *NativeCodegen, args: []ast.Node) CodegenError!void {
    if (args.len > 0) {
        const id = self.nextNameId();
        try self.emitFmt("__m{d}_normcase: {{ const path = ", .{id});
        try self.genExpr(args[0]);
        try self.emitFmt("; var result = __global_allocator.alloc(u8, path.len) catch break :__m{d}_normcase path; for (path, 0..) |c, i| {{", .{id});
        try self.emit(" result[i] = if (c >= 'A' and c <= 'Z') c + 32 else if (c == '/') '\\\\' else c; } break :__m");
        try self.emitFmt("{d}_normcase result; }}", .{id});
    } else try self.emit("\"\"");
}
