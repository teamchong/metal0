...
pub const Funcs = std.StaticStringMap(h.H).initComptime(.{
    ...
    .{ "getuser", h.c("getuser_blk: { const user = if (comptime @import(\"builtin\").os.tag == .windows) \"unknown\" else (std.posix.getenv(\"USER\") orelse std.posix.getenv(\"LOGNAME\") orelse \"unknown\"); break :getuser_blk user; }") },
