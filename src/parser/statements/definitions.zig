    ...
    if (self.tokens[self.current].type == .LParen) {
        ...
        while (self.current < self.tokens.len and paren_depth > 0) {
            ...
            switch (tok.type) {
                ...
                .Is => try type_buf.appendSlice(self.allocator, " is "),
                .Not => try type_buf.appendSlice(self.allocator, "not "),
                .And => try type_buf.appendSlice(self.allocator, " and "),
                .Or => try type_buf.appendSlice(self.allocator, " or "),
                .In => try type_buf.appendSlice(self.allocator, " in "),
                .Plus => try type_buf.append(self.allocator, '+'),
                .Minus => try type_buf.append(self.allocator, '-'),
                .Star => try type_buf.append(self.allocator, '*'),
                .DoubleStar => try type_buf.appendSlice(self.allocator, "**"),
                .Slash => try type_buf.append(self.allocator, '/'),
                .DoubleSlash => try type_buf.appendSlice(self.allocator, "//"),
                .Percent => try type_buf.append(self.allocator, '%'),
                .At => try type_buf.append(self.allocator, '@'),
                else => break,
            }
            self.current += 1;
        }
