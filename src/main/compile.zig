...
/// Emit only @logic_table structs - compiles to static library (.a)
/// The .a includes metal0 runtime and can be linked directly into other projects
pub fn compileLogicTableOnly(allocator: std.mem.Allocator, input_file: []const u8, output_file_opt: ?[]const u8) !void {
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const aa = arena.allocator();
    ...
    // Codegen - generate full code first, then extract logic_table structs
    ...
    // Extract only @logic_table structs (pub const X = struct { pub const __logic_table__ = true; ... };)
    var result = std.ArrayListUnmanaged(u8){};

    // Add header for static library
    ...
    // Find and extract all structs with __logic_table__ = true
    var pos: usize = 0;
    while (std.mem.indexOfPos(u8, full_code, pos, "pub const __logic_table__ = true;")) |marker_pos| {
        // Find the start of this struct (search backwards for "pub const X = struct {")
        var struct_start = marker_pos;
        while (struct_start > 0) {
            if (std.mem.startsWith(u8, full_code[struct_start..], "pub const ")) {
                // Check if this is a struct definition
                if (std.mem.indexOf(u8, full_code[struct_start..marker_pos], " = struct {")) |_| {
                    break;
                }
            }
            struct_start -= 1;
        ...
        // Find the end of this struct by counting braces from struct_start
        var depth: i32 = 0;
        var struct_end = struct_start;
        var found_open = false;
        while (struct_end < full_code.len) {
            if (full_code[struct_end] == '{') {
                depth += 1;
                found_open = true;
            } else if (full_code[struct_end] == '}') {
                depth -= 1;
                if (found_open and depth == 0) {
                    struct_end += 1; // Include the closing brace
                    // Skip the semicolon if present
                    if (struct_end < full_code.len and full_code[struct_end] == ';') {
                        struct_end += 1;
                    }
                    break;
                }
    ...
    const output_path = if (output_file_opt) |path| blk: {
        // Ensure .a extension
        if (std.mem.endsWith(u8, path, ".a")) {
            break :blk try aa.dupe(u8, path);
        } else if (std.mem.endsWith(u8, path, ".zig")) {
            // Replace .zig with .a
            break :blk try std.fmt.allocPrint(aa, "{s}.a", .{path[0 .. path.len - 4]});
        } else {
            break :blk try std.fmt.allocPrint(aa, "{s}.a", .{path});
        }
    } else blk: {
        const stem = build_dirs.getBaseName(input_file);
        break :blk try std.fmt.allocPrint(aa, "{s}_logic_table.a", .{stem});
    };

    // Compile to static library
    try compiler.compileZigStaticLib(allocator, result.items, output_path);
    std.debug.print("✓ Generated static library: {s}\n", .{output_path});
}
...
pub fn compileFile(allocator: std.mem.Allocator, opts: CompileOptions) !void {
    ...
    // Emit only @logic_table structs (no runtime, for LanceQL integration)
    if (opts.emit_logic_table) {
        return try compileLogicTableOnly(allocator, opts.input_file, opts.output_file);
    }
