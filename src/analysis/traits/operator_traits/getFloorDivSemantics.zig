...
//! RETURNS: .zig_native (int), .python_floored (float), .runtime_dispatch (unknown/fraction)
...
/// Check if type is Fraction (requires runtime dispatch for floor division)
fn isFractionType(t: NativeType) bool {
    return t == .fraction;
}
...
pub fn getFloorDivSemantics(left: NativeType, right: NativeType) OperatorSemantics {
    ...
    // Fraction types need runtime dispatch (Fraction.floordiv method)
    if (isFractionType(left) or isFractionType(right)) return .runtime_dispatch;
